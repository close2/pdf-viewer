...
//! what `raster_scene::function::FnOp` quotes.
//!
...
pub enum Table42 {
    ...
    /// `xor`.
    Xor,
    /// `if`, which reaches us lowered to [`raster_scene::function::FnOp::JumpUnless`].
    If,
    /// `ifelse`, which reaches us lowered to [`raster_scene::function::FnOp::JumpUnless`]
    /// and [`raster_scene::function::FnOp::Jump`].
    Ifelse,
    /// `copy`.
    Copy,
    /// `dup`.
