...
use raster_function_conformance::case::{Expectation, Refusal, Subject, Tolerance};
use raster_function_conformance::reference::error::EvalError;
use raster_function_conformance::table42::Table42;
use raster_function_conformance::{Case, Evaluation, corpus, evaluate_case};
...
fn check_reports(
    evaluation: &Evaluation,
    expected: Option<raster_function_conformance::Report>,
) -> Result<(), String> {
...
fn the_two_input_adaptor_moves_jump_targets() {
    for case in corpus::cases() {
        ...
        for (original, moved) in case.program.iter().zip(adapted.iter().skip(prefix)) {
            let shift = |op: &raster_scene::function::FnOp| match *op {
                raster_scene::function::FnOp::Jump { target }
                | raster_scene::function::FnOp::JumpUnless { target } => Some(target),
                _ => None,
            };
...
fn the_two_input_adaptor_preserves_what_a_case_computes() {
    for case in corpus::cases() {
        ...
        let evaluation =
            raster_function_conformance::evaluate(&adapted, &inputs, case.domain, case.range);
        match evaluation {
