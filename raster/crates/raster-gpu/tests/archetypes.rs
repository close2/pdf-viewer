...
//!
//! **The pages themselves are `raster-pages`** (ADR 0060). They were defined in this
...
use raster_scene::{Affine, ImageId, OutlineId, Scene};

const WIDTH: u32 = 1191;
const HEIGHT: u32 = 1684;

/// The archetype's resources, uploaded in the order `raster_pages::outlines` states.
fn upload(device: &mut Device, shape: &Archetype) -> (Vec<OutlineId>, Option<ImageId>) {
    let outlines = raster_pages::outlines(shape)
        .iter()
        .map(|path| device.upload_outline(path).unwrap())
        .collect();
    let image = raster_pages::image_spec(shape)
...
/// is not. `raster-pages` cannot own this function: `Counters` lives here, and that
...
///
/// The recorded rows are `raster-pages`' — each page's doc comment explains its own row
...
fn the_archetypes_cost_what_they_are_recorded_to_cost() {
    ...
    assert!(
        ...
        "the archetype signature moved. Every field is an exact function of the scene \
         ...
         then record it in `raster-pages`:\n  {}",
        moved.join("\n  ")
    );
}
...
///   it. This is the property `raster_pages::curve_clip` is written to hold.
...
/// `cargo test --release -p raster-gpu --test archetypes -- --ignored --nocapture`.
