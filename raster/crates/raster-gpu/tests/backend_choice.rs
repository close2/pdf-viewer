...
use raster_gpu::{Device, DeviceError, Options, wgpu};
...
fn a_named_backend_is_the_backend_the_device_uses() {
    let instance = raster_gpu::create_instance_with(wgpu::Backends::VULKAN);
...
fn restricting_the_set_only_removes() {
    ...
    let vulkan =
        Device::adapter_names_on(&raster_gpu::create_instance_with(wgpu::Backends::VULKAN));
    for name in &vulkan {
        assert!(
            all.contains(name),
...
fn a_set_this_machine_cannot_supply_refuses_by_name() {
    let instance = raster_gpu::create_instance_with(wgpu::Backends::empty());
    assert!(
...
fn the_unrestricted_entry_point_is_unchanged() {
    let default = Device::adapter_names_on(&raster_gpu::create_instance());
    let explicit =
        Device::adapter_names_on(&raster_gpu::create_instance_with(wgpu::Backends::all()));
    assert_eq!(
        default, explicit,
