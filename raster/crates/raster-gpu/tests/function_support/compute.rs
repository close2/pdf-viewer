...
//! `raster/doc/spike-function-paint.md` section 5 measured 246 044 texels off by one from ADR
//! 0006's 8-bit store *alone*, so a raster would put that conversion between the shader and every
...
use raster_gpu::function::{background_rgba, generate, range_bounds};
use raster_gpu::wgpu;
use raster_scene::{Color, FnOp, FnRange};
...
fn evaluate_points(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    colours[index] = raster_function_evaluate(
...
/// What one program is evaluated under: everything `raster_function_evaluate` takes that is
/// not the point.
...
impl Compute {
    ...
    pub fn new() -> Option<Self> {
        let instance = raster_gpu::create_instance();
        ...
        let (device, queue) = pollster::block_on(adapter.request_device(&wgpu::DeviceDescriptor {
            label: Some("raster function tests"),
    ...
    pub fn run(&self, shading: &Shading<'_>, points: &[(f32, f32)]) -> Vec<[f32; 4]> {
        ...
        let module = self
            ...
            .create_shader_module(wgpu::ShaderModuleDescriptor {
                label: Some("raster generated function"),
