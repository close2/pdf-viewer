...
//! reference evaluator.
//!
//! `raster-function-conformance` holds 125 cases over all 42 of Table 42's operators, each
...
use raster_function_conformance::case::{Case, Expectation, Refusal, Subject};
use raster_function_conformance::{cases, evaluate_case};
use raster_gpu::error::FunctionProblem;
use raster_gpu::function::FunctionRefusal;
use raster_gpu::{Device, Options};
use raster_scene::{FnOp, FnRange};
...
fn upload(device: &mut Device, case: &Case) -> Disposition {
    match device.upload_function(&case.two_input_program()) {
        ...
        Err(raster_gpu::DeviceError::InvalidFunction { reason }) => Disposition::Refused(reason),
        Err(other) => panic!(
...
fn refused_with_its_range(case: &Case) -> Option<FunctionRefusal> {
    ...
    let analysis = raster_gpu::function::analyse(&program).ok()?;
...
fn names_the_ground(refusal: &FunctionProblem, ground: Refusal) -> bool {
    matches!(
        ...
        (
            ...
            FunctionProblem::Structure(raster_scene::SceneError::BackwardFunctionJump { .. }),
            Refusal::BackwardJump,
        ) | (
            FunctionProblem::Structure(raster_scene::SceneError::FunctionJumpOutOfRange { .. }),
...
fn the_device_computes_what_the_corpus_evaluator_computes() {
    ...
    for case in cases() {
        ...
        let program = case.two_input_program();
        let analysis = match raster_gpu::function::admit(&program) {
...
fn every_refusal_the_device_takes_is_one_of_the_stated_reasons() {
    ...
    for case in cases() {
        ...
        let program = case.two_input_program();
        let Err(reason) = raster_gpu::function::admit(&program) else {
            continue;
        };
        taken += 1;
