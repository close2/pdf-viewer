...
//! `raster-scene`'s `validate.rs` and `frames.rs` cross every scene-boundary refusal and
...
use raster_gpu::{Device, Options, RenderError, Target, Viewport};
use raster_scene::{
...
/// `MAX_COORDINATE` (`raster-scene`'s `validate.rs`), and an outline point above it
...
fn no_batch_is_noted_before_its_instance_is_written() {
    ...
    builder
        .rect(
            raster_scene::Rect::new(Point::new(8.0, 8.0), Point::new(24.0, 24.0)),
            Affine::IDENTITY,
            Color::new(0.0, 0.0, 0.0, 1.0),
            None,
            None,
