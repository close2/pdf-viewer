...
//! the one `raster/doc/notes-function-wiring.md` section 2.3 quotes:
//!
...
//! `function_ops.wgsl` (a zero divisor, a negative `sqrt`), and ADR 0053 section 3.2 already has
...
/// never equal, and `raster/doc/notes-function-wiring.md` section 2.2 records the corpus case that
/// found us answering the opposite. It is asserted here beside the unpinned three so that a
