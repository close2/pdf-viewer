...
const SHIPPING: [&str; 3] = ["raster", "raster-gpu", "raster-scene"];
...
/// *specific* new name and the message should be able to say which. `raster-scene`'s
...
/// — cargo does not separate them there — so following `raster-gpu` through the lock would
...
fn the_scene_crate_has_no_dependencies_at_all() {
    ...
    .expect("raster-scene's manifest");
    assert_eq!(
