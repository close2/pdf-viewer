...
//!
//! [`Limits::atlas_bytes`]: raster_gpu::device::Limits::atlas_bytes
...
fn the_budget_is_a_request_and_the_limit_is_what_it_bought() {
    let ordinary = device(raster_gpu::DEFAULT_ATLAS_BUDGET);
    let ceiling = ATLAS_WIDTH_CAP.saturating_mul(u64::from(ordinary.limits().max_target_size));
    assert!(
        raster_gpu::DEFAULT_ATLAS_BUDGET <= ceiling,
    ...
    assert_eq!(
        ...
        raster_gpu::DEFAULT_ATLAS_BUDGET,
...
fn a_frame_says_how_many_marks_the_full_atlas_cost_it() {
    let mut roomy = device(raster_gpu::DEFAULT_ATLAS_BUDGET);
