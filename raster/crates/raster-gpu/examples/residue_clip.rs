...
/// The page: `raster_pages::ARTWORK`, the same definition `tests/archetypes.rs` gates.
///
...
fn build(device: &mut Device) -> Scene {
    let outlines: Vec<raster_scene::OutlineId> = raster_pages::outlines(SHAPE)
        .iter()
        .map(|path| device.upload_outline(path).expect("an archetype outline"))
        .collect();
    raster_pages::scene(SHAPE, &outlines, None).expect("the artwork archetype builds")
}

/// The frame's counters as the row `raster-pages` records, field by named field.
...
fn main() {
    ...
    for round in 0..=rounds {
        ...
        if round == 0 {
            ...
            // the row it compares against is `raster-pages`', the same one
            ...
            assert_eq!(
                ...
                "this frame is not the artwork archetype as `raster-pages` records it",
            );
