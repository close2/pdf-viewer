...
use raster_gpu::{Counters, Device, EncodeSource, Options, RetainedScene, Target, Viewport, wgpu};
...
/// A frame's counters as the row `raster-pages` records, field by named field.
///
...
fn build(device: &mut Device) -> Scene {
    let outlines: Vec<OutlineId> = raster_pages::outlines(SHAPE)
        .iter()
        .map(|path| device.upload_outline(path).expect("an archetype outline"))
        .collect();
    raster_pages::scene(SHAPE, &outlines, None).expect("the dense-text archetype builds")
}
...
/// The overflow section's page: **`raster_pages::GLYPH_PAGE`** — 5 933 fills over 107
...
fn overflow_page(device: &mut Device) -> Scene {
    let outlines: Vec<OutlineId> = raster_pages::glyph_outlines(OVERFLOW_PAGE)
        .iter()
        .map(|path| device.upload_outline(path).expect("a letterform"))
        .collect();
    raster_pages::glyph_scene(OVERFLOW_PAGE, &outlines).expect("the glyph page builds")
}
...
fn overflow_section(adapter: Option<&str>, frames: usize) {
    ...
    // `raster_pages::zoomed` — `examples/zoom.rs` asks for the same frame through the
    // same function, so the two examples magnify one page about one point.
    let viewport = Viewport::full(WIDTH, HEIGHT, raster_pages::zoomed(OVERFLOW_PAGE, 4.0));
...
fn round_robin(run: &mut RoundRobin<'_>) -> (Vec<Record>, Vec<Record>, Counters) {
    ...
    let record = |wall, frame: &raster_gpu::Frame| Record {
        wall,
