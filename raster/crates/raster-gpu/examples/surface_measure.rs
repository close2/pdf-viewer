...
//!
//! ```text
//! MESA_SHADER_CACHE_DISABLE=true cargo run --release -p raster-gpu --example surface_measure
//! ```
//!
...
use raster_gpu::{Device, Options, Target, Viewport, WarmUp};
use raster_pages::{ARTWORK, Archetype, DENSE_TEXT};
...
fn build(device: &mut Device, shape: &Archetype) -> Scene {
    let outlines: Vec<OutlineId> = raster_pages::outlines(shape)
        ...
        .collect();
    raster_pages::scene(shape, &outlines, None).expect("an archetype builds")
}

/// A frame's counters as the row `raster-pages` records, field by named field.
fn recorded(counters: &raster_gpu::Counters) -> raster_pages::Recorded {
    raster_pages::Recorded {
...
impl FrameRecord {
    fn of(wall: Duration, timings: &raster_gpu::Timings) -> Self {
        let phase = |name: &str| {
            timings
                .phases
                .iter()
            ...
            event_loop
                .create_window(
                    Window::default_attributes()
                        .with_title("raster surface measure")
