...
use raster_gpu::{Device, Layer, RenderError, Target, Viewport};
use raster_pages::DENSE_TEXT;
use raster_scene::{Affine, Color, ImageFilter, Point, Rect, Scene, SceneBuilder};
...
fn present_run(
    display: &mut Display,
    presenter: &mut raster_gpu::Presenter,
    layers: &[Layer<'_>],
    count: usize,
) -> Run {
...
fn dense_text(device: &mut Device) -> Scene {
    let outlines: Vec<raster_scene::OutlineId> = raster_pages::outlines(&DENSE_TEXT)
        .iter()
        .map(|path| device.upload_outline(path).expect("an archetype outline"))
        .collect();
    raster_pages::scene(&DENSE_TEXT, &outlines, None).expect("an archetype builds")
}
