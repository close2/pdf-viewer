...
use raster_scene::{Affine, Color, Point, Rect, Scene, SceneBuilder};
...
/// cannot help).
///
/// **`raster_pages::GLYPH_PAGE`** — the definition `examples/zoom.rs` and
...
fn glyph_page(device: &mut Device, unique_phases: bool) -> Scene {
    let page = if unique_phases {
        raster_pages::GLYPH_PAGE_UNIQUE_PHASES
    } else {
        raster_pages::GLYPH_PAGE
    };
    let outlines: Vec<raster_scene::OutlineId> = raster_pages::glyph_outlines(&page)
        .iter()
...
fn figure_resources(
    ...
    raster_scene::ImageId,
    raster_scene::RampId,
    raster_scene::MeshId,
    raster_scene::OutlineId,
) {
    use raster_scene::{ImageSpec, MeshSpec, Segment, Stop};
...
fn figure_page(device: &mut Device) -> Scene {
    use raster_scene::{BlendMode, Compose, FillRule, ImageFilter, Paint, ShadingKind};
