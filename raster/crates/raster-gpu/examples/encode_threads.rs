...
//!
//! ```text
//! cargo run --release -p raster-gpu --example encode_threads \
...
use raster_pages::{ARTWORK, Archetype, CALLERS_DRAWING, DENSE_TEXT_UNCLIPPED, MEDIAN_PAGE};
use raster_scene::{Affine, OutlineId, Scene};

/// The four page shapes this sweep runs, all four defined in `raster-pages` (ADR 0060).
///
...
///   40 tiles of **8 956** coverage texels (`raster_pages::DENSE_TEXT`'s recorded row);
...
fn build(device: &mut Device, shape: &Archetype) -> Scene {
    let outlines: Vec<OutlineId> = raster_pages::outlines(shape)
        ...
        .collect();
    raster_pages::scene(shape, &outlines, None).expect("a page builds")
}
