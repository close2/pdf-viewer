...
impl Modules {
    ...
    fn new(device: &wgpu::Device) -> Result<Self, PipelineProblem> {
        ...
        Ok(Self {
            rect: module("raster rect", shaders::RECT)?,
            cover: module("raster coverage", shaders::COVERAGE)?,
            image: module("raster image", shaders::IMAGE)?,
            shading: module("raster shading", shaders::SHADING)?,
            composite: module("raster composite", shaders::COMPOSITE)?,
            reduce: module("raster reduce", shaders::REDUCE)?,
            blit: module("raster blit", shaders::BLIT)?,
            present: module("raster present", shaders::PRESENT)?,
            winding: module("raster winding", shaders::WINDING)?,
        })
    }
}
