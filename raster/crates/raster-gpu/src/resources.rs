...
pub(crate) struct CollapsedMark {
    ...
    pub min: raster_scene::Point,
    /// The corner with the largest.
    pub max: raster_scene::Point,
...
fn collapsed_marks(path: &[Segment]) -> Box<[CollapsedMark]> {
    ...
    let mut opened_at: Option<raster_scene::Point> = None;
    while index < path.len() {
        let mut bounds: Option<(raster_scene::Point, raster_scene::Point)> = None;
        let include =
            |p: raster_scene::Point,
             bounds: &mut Option<(raster_scene::Point, raster_scene::Point)>| {
                match bounds {
                    Some((min, max)) => {
                        min.x = min.x.min(p.x);
                        min.y = min.y.min(p.y);
...
impl ResourceStore {
    ...
    pub(crate) fn upload_outline(&mut self, path: &[Segment]) -> Result<OutlineId, DeviceError> {
        ...
        for segment in path {
            let points: &[raster_scene::Point] = match segment {
...
mod tests {
    ...
    use raster_scene::{ImageSpec, Point, ResourceId, Segment, Stop};
    ...
    fn ramp_validation_names_each_refusal() {
        let mut store = ResourceStore::new(u64::MAX);
        let color = raster_scene::Color::new(0.0, 0.0, 0.0, 1.0);
        assert!(matches!(
            store.upload_ramp(&[]),
            Err(DeviceError::InvalidResource {
                reason: ResourceProblem::RampEmpty
    ...
    fn one_id_space_prevents_cross_family_aliasing() {
        ...
        assert!(matches!(
            store.release(ResourceId::Image(raster_scene::ImageId(outline.0))),
            Err(DeviceError::UnknownResource { .. })
        ));
    }
}
