...
impl Layouts {
    ...
    pub(crate) fn new(device: &wgpu::Device) -> Self {
        ...
        let lane_pipe = pipe_layout("raster lane", &[&layouts.globals, &layouts.textures]);
        let image_pipe = pipe_layout("raster image", &[&layouts.image]);
        let shading_pipe = pipe_layout("raster shading", &[&layouts.shading]);
        let function_pipe = pipe_layout("raster function", &[&layouts.function]);
        let composite_pipe = pipe_layout("raster composite", &[&layouts.composite]);
        let reduce_pipe = pipe_layout("raster reduce", &[&layouts.reduce]);
        let blit_pipe = pipe_layout("raster blit", &[&layouts.blit]);
        let present_pipe = pipe_layout("raster present", &[&layouts.present]);
        let winding_pipe = pipe_layout("raster winding", &[&layouts.winding]);
        let resolve_pipe = pipe_layout(
            "raster winding resolve",
...
fn bind_layouts(device: &wgpu::Device) -> BindLayouts {
    ...
    BindLayouts {
        ...
        sampled: make("raster sampled texture", &[texture_entry(0)]),
        ...
        winding: make("raster winding", &[uniform_entry(0, 48, QUAD_UNIFORM)]),
    }
}
