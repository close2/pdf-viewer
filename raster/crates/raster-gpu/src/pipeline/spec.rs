...
impl<'a> Spec<'a> {
    ...
    pub(crate) fn of(kind: Kind, layouts: &'a Layouts, modules: &'a Modules) -> Self {
        match kind {
            ...
            Kind::Reduce => {
                Self::full_screen("raster reduce", &modules.reduce, &layouts.reduce_pipe)
            }
            Kind::Blit => Self::full_screen("raster blit", &modules.blit, &layouts.blit_pipe),
