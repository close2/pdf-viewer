...
use raster_scene::{Affine, BlendMode, ClipId, Compose, FillRule, MaskId, OutlineId, Paint};
...
fn snapped_mark(
    ...
    let (lo, hi) = if x0 == x1 && y0 != y1 {
        let column = x0.floor();
        (
            raster_scene::Point::new(column, y0),
            raster_scene::Point::new(column + 1.0, y1),
        )
    } else if y0 == y1 && x0 != x1 {
        let row = y0.floor();
        (
            raster_scene::Point::new(x0, row),
            raster_scene::Point::new(x1, row + 1.0),
        )
    } else {
        return None;
    };
    ...
    Some(raster_scene::Rect::new(lo, hi))
}
        ...
        for mark in marks {
            ...
            if let Some(rect) = device_rect {
                ...
                self.push_mark_corners(
                    [
                        rect.min,
                        raster_scene::Point::new(rect.max.x, rect.min.y),
                        rect.max,
                        raster_scene::Point::new(rect.min.x, rect.max.y),
                    ],
                    resolved,
                    ink,
                    style,
            ...
            let (lo, hi) = if mark.vertical {
                (
                    raster_scene::Point::new(mark.min.x - half, mark.min.y),
                    raster_scene::Point::new(mark.min.x + half, mark.max.y),
                )
            } else {
                (
                    raster_scene::Point::new(mark.min.x, mark.min.y - half),
                    raster_scene::Point::new(mark.max.x, mark.min.y + half),
                )
            };
            self.push_mark_corners(
                [
                    apply(to_device, lo),
                    apply(to_device, raster_scene::Point::new(hi.x, lo.y)),
                    apply(to_device, hi),
                    apply(to_device, raster_scene::Point::new(lo.x, hi.y)),
                ],
                resolved,
                ink,
                style,
...
pub(super) fn corner_bounds(
    ...
    let corners = [
        apply(
            to_device,
            raster_scene::Point::new(control_box[0], control_box[1]),
        ),
        apply(
            to_device,
            raster_scene::Point::new(control_box[2], control_box[1]),
        ),
        apply(
            to_device,
            raster_scene::Point::new(control_box[0], control_box[3]),
        ),
        apply(
            to_device,
            raster_scene::Point::new(control_box[2], control_box[3]),
        ),
    ];
