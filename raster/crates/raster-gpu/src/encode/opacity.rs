...
use raster_scene::{Command, Paint};
...
///
/// [`MAX_GROUP_DEPTH`]: raster_scene::MAX_GROUP_DEPTH
...
mod tests {
    ...
    fn fill(paint: Paint, mask: Option<MaskId>, compose: Compose) -> Command {
        Command::Fill {
            outline: raster_scene::OutlineId(0),
            transform: Affine::IDENTITY,
    ...
    fn a_paint_whose_colours_are_uploaded_is_not_provable() {
        let shading = Paint::Shading {
            ramp: raster_scene::RampId(0),
            kind: raster_scene::ShadingKind::Axial {
        ...
        assert!(!every_opacity_is_one(&[fill(
            Paint::Mesh(raster_scene::MeshId(0)),
            None,
            Compose::SrcOver
        )]));
    }
