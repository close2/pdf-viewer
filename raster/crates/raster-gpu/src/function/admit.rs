...
//!
//! 1. **Is the instruction list executable at all?** `raster_scene::check_program` — a
//!    non-empty list, within the vocabulary's own length bound, every jump strictly
//!    forward and inside. It belongs to `raster-scene` because it is about the vocabulary
...
pub fn admit(program: &[FnOp]) -> Result<Analysis, FunctionProblem> {
    raster_scene::check_program(program).map_err(FunctionProblem::Structure)?;
    ...
    fn an_empty_program_is_refused_by_the_structural_check() {
        assert!(matches!(
            ...
            Err(FunctionProblem::Structure(
                raster_scene::SceneError::EmptyFunctionProgram
            ))
        ));
    }
