...
//!
//! ```ignore
//! let outlines: Vec<OutlineId> = raster_pages::outlines(&raster_pages::ARTWORK)
//!     .iter()
//!     .map(|path| device.upload_outline(path).expect("an archetype outline"))
//!     .collect();
//! let scene = raster_pages::scene(&raster_pages::ARTWORK, &outlines, None)
...
//! `Counters` lives in `raster-gpu`, which this crate must not depend on. Each consumer
