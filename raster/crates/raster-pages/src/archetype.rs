...
use raster_scene::{Affine, Point, Segment};
...
/// from its own `Counters` (which lives in `raster-gpu`, a crate this one must not
...
/// An axis-aligned rectangle as a path. `raster_scene`'s own detector recognises this
...
/// ADR 0049 and ADR 0057, and did not (`raster/doc/notes-tiling-bound.md` section 3).
#[must_use]
