...
ISO 19005-2 6.4.1, ISO 19005-4 6.4.1 · all six · **built** (ADR 1257)
...
- **Built** (ADR 1257), in both shapes, with `construct = true` the default because Table 224 makes
  removing the flag a *claim* that streams have been provided for every visible widget. The
  population is that obligation's own — `pdf_archive::visible_widgets`: visible by §12.5.3's Table
  167, and with no `/N` by §12.5.5's Table 170, so a widget its producer already gave a stream keeps
  the producer's bytes. A button field's widget is never constructed for (§12.7.5.2.3 makes its `/N`
  a subdictionary of states its value selects among), and a widget whose appearance cannot be
  completed leaves the site refused with its **field** named in the report's `unconstructed`, rather
  than clearing a flag the file would then be lying with. `construct = false` is the other answer
  and costs `Loss::FieldAppearances`.

#### `forms/no-xfa-key`
#### `forms/no-needs-rendering`
ISO 19005-2 6.4.2, ISO 19005-4 6.4.2 · all six · `forms/no-needs-rendering`
**built in session 962**, `forms/no-xfa-key` **built** (ADR 1257)
...
- **Built** (ADR 1257) as `Loss::XfaForm`, and **the predicate is the document's own sentence**
  rather than a reading of the XFA resource: Annex K states no test and says outright that whether
  an XFA schema's pages are materialised as PDF pages is implementation dependent, so the pages
  settle nothing either — while §7.7.2's Table 29 makes `/NeedsRendering` true the claim that the
...
ISO 19005-2 6.8, ISO 19005-4 6.9 · today **`derive` built, `discard` built** (ADR 1258)
...
- **From a configuration** — the RFC's own example, and it is expressible: `remedy = "derive"` with
  ...
  **Built since session 999**, in the third of those four shapes and only that one: a declared tool
  ...
  The first and third of the four shapes arrive by the target rather than by code, and **the fourth
  of the catalogue's list, `discard`, is built** (ADR 1258): every reference to the file
  specification goes — its entry in the `/EmbeddedFiles` name tree and its place in every `/AF`
  array — and the specification and the stream under it go with them, because the serializer copies
  what the converted document reaches. §7.7.4's Table 31 is why both entries go rather than one, and
  the report names each file that left. Two shapes are refused by name rather than half-answered: a
  specification a file attachment annotation's `/FS` or an embedded `GoTo` still reaches, and one
  the name tree writes directly rather than as an object. **Appending the attachment's pages is
  still not built**, and its blocker is now only the page composition: the name-tree rewrite it also
  needed is this one.
