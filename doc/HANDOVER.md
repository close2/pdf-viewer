...
| Transparency departures (§11.4.4, §11.4.7, §11.6.6) — **§11.5.3's population closed in the three-hundred-and-eighty-third** (ADRs 0217, 0220), **§11.4.6's shape in the three-hundred-and-ninety-seventh** (ADR 0234) and **§11.4.4's non-isolated group in the four-hundredth** (ADR 0237). What stands is one raster format — a blending space that is not the device's three components, all `/DeviceCMYK` — plus §11.4.6's knockout where the elements blend. **The four-hundred-and-fifteenth found that population was the wrong one**: a group's `/CS` binds only where the group is isolated (§11.6.6, §11.7.2) and §11.4.7's page group is the root of the inheritance, which nothing here read — so one report was false and five documents departed in silence (ADR 0251). Reports with no corpus member sit inside the closed ones, `/AIS` among them | 9 | [todo 23](todo/23-transparency-departures.md) |
