...
| **quorra vs the CPU oracle** (974 documents, page one, same display list) | **FAILING since session 310 and nobody ran it**: six pages refused where the ratchet holds one, the five new ones all "over the stated budget" of 256 MiB of scene-derived bytes — [todo 12](todo/12-five-pages-quorra-newly-refuses.md), found in the three-hundred-and-eleventh and not caused by it (checked with that session's changes stashed). The last figures it printed clean were **913 agree, 43 differ, 1 refused**, 17 not comparable — 28 of the 43 are the two rasterisers' glyph antialiasing and not a defect list (ADR 0156) | `render-quorra/tests/corpus.rs`, **27 s** |
