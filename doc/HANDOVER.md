...
4.1 MB of pixels down a pipe. **All twenty-nine questions cross**: twenty-five since the three-hundred-and-eighty-sixth,
...
confined host build native controls rather than take a form as pixels (ADR 0235); and **§12.5.6.6's
free text at a point since the four-hundred-and-first** (ADR 0238), which is the twenty-ninth and
which this sentence did not count for forty-four rounds — it said twenty-eight until the
four-hundred-and-forty-fifth counted `query_kind`'s discriminants and `viewer_core::Query`'s
variants, which are 29 and 29, against the round-trip test's 29. The largest of them is ISO 32000-2's
...
**The four-hundred-and-forty-fifth is a closing round — the last of the block of thirty — and it ran
everything, not only what a round touches.** Every count in the table below reproduced exactly:
tests **1619** with 11 skipped and one doctest beside them, corpus **974 with 65 incomplete**, oracle
**1794 at 1693/101** with 905 / 68 / 786, geometry 1 and 2, not comparable 14, no render 18 and the
undiagnosed list **empty**, quorra **917 / 35 / 5 / 17**, text 99.2% (24007/24191) with 23 below the
floor, PDFBox 99.8% (14257/14281) with 4 below, dates 1514/1545, XMP 318/1 with 3191 properties,
JPEG 2000 14 byte-identical, and conformance **6559 citations, 631 quotations, 219 tables, 875 rows
at 410/244/18/82/8/113** with 0 unreviewed. **What did not reproduce is three of the ten timings**,
which is the paragraph under the table.

**Everything beyond §2 was run and every one of them passed**, which is the first round in this
block to be able to say so of all of them at once: `cargo deny check` — *advisories ok, bans ok,
licenses ok, sources ok*; **all five** cross-target checks under `RUSTFLAGS="-D warnings"`;
`cargo build --release -p pdf-sandbox --bins` (trap 10); `rustfmt --check` and `cargo clippy
--all-targets` inside the fuzz workspace, which neither §2 gate reaches; **all fourteen fuzz targets
at their stated counts** — eleven at 50 000 runs, `page` at **50 045 iterations over six forks in
4595 s** after merging 39 012 seeds to 9072 of distinct coverage (34 551 edges, corpus 9890), and
`confined_wire` and `x509` at 1 000 000 apiece in 19 s and 5 s — **with 0 crashes, 0 leaks and 0
out-of-memory across all fourteen**. `page` left 19 new `slow-unit-` artefacts and nothing else, and
two of the largest render in **0.79 s and 0.70 s** against a release build, which is §2's own
prediction about what a `slow-unit-` under ASan and six forks means; `spec-errata check`
over the fourteen documents, **151 and 27**, unmoved for eleven rounds; and **the window under
`Xvfb`**, where ISO 32000-2 launches to its first present in **122.6 ms** with §12.3.3's sidebar open
and five arrow keys turn to page 6 presenting in **9.2 to 16.1 ms**.

**What a closing round is for is the numbers that were never printed, and it found twenty-eight of
them across eleven files — nine documents and two rustdoc comments.** The largest, in the order they bite: `doc/HANDOVER.md`'s own "what is not implemented" table said **24
codes over 8 documents** reach no glyph in silence where the corpus gate prints **5 over 2**, and
said the web's largest transparency population was **28** where the same file's session-440
paragraph says 31 and session 441 took it to **0**; the confined transport carries **twenty-nine**
questions and four documents and two doc comments said twenty-eight or twenty-five, uncounted since
§12.5.6.6's arrived in the four-hundred-and-first; `doc/todo/02` §2's own arithmetic said
`cargo test --workspace` reports **1551** where it reports **1620**, and opened a paragraph "Twelve
fuzz targets" two paragraphs under one saying fourteen; `doc/todo/README.md`'s index said §10.5's
transfer function is ignored and `silent` — **eighty-seven rounds** after it was implemented — and
carried *5 names* in the ambiguous bucket sixty-six rounds after that list went to zero;
`doc/verify.md` said `spec-errata check` prints **79 and 3** where it prints 151 and 27, and that
`Tree::walk` stops at 65 536 items, which ADR 0257 made 2²⁰. **None of them is a defect in the
program and that is the point**: every gate ratchets, and no gate reads a sentence.

**The four-hundred-and-forty-fourth changed the correctness oracle's own rasteriser and every
summary line below reproduced.** It composed a clip *chain* as §10.7.4's intersection of sets rather
than as a product of anti-aliased coverages (ADR 0280), so the tests moved by its three
(1616 → **1619**, 11 skipped), the citations by five (6554 → **6559**) and the quotations by one
(630 → **631**), while the corpus's 974 with 65 incomplete, the oracle's 1794 at 1693/101 with
905 / 68 / 786, **quorra's 917/35/5/17 line for line**, both text gates, the dates, the XMP and the
JPEG 2000 lines are what the table below says. **22 of the oracle's 1794 per-page lines moved and
none changed verdict** — every movement in the third or fourth decimal place, which is what a change
confined to pixels where two clip boundaries fall looks like — and `doc/todo/00`'s step 7 was run
before and after over all 786 ambiguous pages with **the negative tail byte-identical**: twenty names
at or past −1, sixteen of them incomplete, and on the complete documents `issue16038.pdf` −5.734,
`issue12295.pdf` −2.956, `issue14297.pdf` −1.150, `issue7821.pdf` −1.000 and nothing past −0.536.
Twenty-one rows moved and twenty of them up. **Two of the sweep's own names have drifted since it was
last run whole in the four-hundred-and-fifteenth** — `issue16038.pdf` and `issue12295.pdf`, the two
pages ADRs 0213 and 0268 are about — which is `doc/todo/00`'s own lesson holding rather than a
finding. Cost, measured: **+0.19%** of the rasteriser's instructions on ISO 32000-2 page 101 and
**−8.75%** on `bug1721218_reduced.pdf`, the corpus's heaviest clip page, because a chain allocates one
scratch mask where `tiny-skia` allocated one per link. **Run beyond §2 and claimed**: `spec-errata`,
whose §10.7 hits are none — no quotation of that subclause quotes struck text — and
`callgrind_rasterise` twice each way. **Not run and therefore not claimed**: the fourteen fuzz
targets, since no parser, decoder or document code changed; `cargo deny`, since no dependency moved;
the window under `Xvfb`, since nothing a person presses is involved. `doc/QUORRA_FEEDBACK.md` §18 is
the ask the change leaves: the graphics device composes its chain inside the library and still
multiplies.

**The four-hundred-and-forty-third ran the sequence and every line reproduced except two counts** —
citations 6543 → **6554**, quotations 628 → **630**; tests **1616**, 11 skipped, unmoved. The round
worked the contradicted list and moved no pixel, which was checked rather than assumed: **all 1794 of
the oracle's per-page lines are byte-identical before and after**, 905 / 68 / 786 with 1693/101, and
the corpus's 65 incomplete, quorra's 917/35/5/17, both text gates, the dates, the XMP and the
JPEG 2000 lines are what the table below says. What moved is three diagnoses. **`CONTRADICTED_PAGE_ROUNDING`
is empty**: its two pages were filed on `<stem>-p<n>-ours.png`, which is our raster *after*
`normalise::to_common_size` crops it to the smallest voting reference's size, and our own renders are
596 × 842 and 179 × 179 — `poppler`'s and `mupdf`'s — because `TargetSpec::for_page` rounds *up*.
`colorkeymask.pdf` is §10.7.4's image paragraph carried out **against** the consensus (ours byte-identical
to `ghostscript` over 595 × 842; `poppler` averages two samples at a one-to-one placement) and
`issue21346.pdf` is six coincident clip boundaries whose coverages multiply — a ladder of `n` such
boundaries reads 0.5025, 0.2487, 0.1218, 0.0609, 0.0305, 0.0152, and §10.7.4's clipping paragraph, which
neither `oracle.rs` nor its ledger row had ever cited, says the clip is a *set of pixels* intersected
with a *set of pixels*. §10.7.4's row gains that as its **fourth** departure. And the ranking's head,
`issue15716.pdf` at 3.10 nearest against 3.92 furthest, is `CONTRADICTED_SUBSTITUTED_FONT`'s third
mechanism with a closed form: every renderer paints the area *its own* ZapfDingbats states to a fifth of
a percent, with identical advances. ADR 0279, `doc/todo/11` item 4.

**The four-hundred-and-thirty-third ran the whole sequence and every line reproduced except three
counts** — tests 1572 → **1580**, citations 6373 → **6378**, quotations 596 → **597**. It surveyed
**all 65 944 crawled documents on disk**, 145 archives and 93 GB, and fixed the two defects the
survey's own failures named: a `OnceLock` initialiser calling rayon, which **hung three archives**
because rayon runs other jobs while `collect` waits and one of them re-entered the initialiser on
the initialising thread; and a path outside `tiny-skia`'s supersampled 16.16 arithmetic, which
**aborted two archives** in the anti-aliased blitter and is now drawn by the aliased one. **Neither
moves the 974** and that is checkable rather than hoped: the corpus's 68 incomplete, the oracle's
905 / 68 / 786 with 1690/104 and the undiagnosed list empty, quorra's 911/35/11/17, both text gates,
the dates, the XMP and the JPEG 2000 lines are what the table below says. `doc/todo/00`'s step 7 is
**not owed** — no corpus document states a path outside ±8191 device units, which is what those
identical oracle and quorra counts mean, since quorra compares the CPU raster page by page. ADR 0269.
...
**Every number in this table was printed by the gate beside it in the four-hundred-and-forty-fifth
session**, a closing round that ran the whole of `doc/todo/02-every-round.md` §2 and read each figure
off the output. Nothing in it is arithmetic performed here — that has been wrong twice — and nothing
in it is carried forward from a previous round. **Every count reproduced and every timing did not**:
the "where" column was the three-hundred-and-ninety-eighth's until this round re-took it, and three
of the ten roughly doubled — oracle **51.5 → 102.0 s**, quorra **25.1 → 39.0 s**, corpus
**3.2 → 5.0 s** — while `nextest`, the pdf.js text gate, the dates, the XMP and JPEG 2000 are where
they were. **The three that moved are exactly the three that rasterise all 974 first pages**, and the two that
did not are the two that do not — which is a hypothesis and is written as one: §11.4.7 has drawn a
four-component page **twice** since ADR 0262, and quorra has drawn its pair since ADR 0275. It is
recorded rather than diagnosed, because no gate measures a gate; `doc/todo/43` is where the project's
own turnaround lives and this is the first number it has been handed that is not a build. The PDFBox row's **0.4 → 3.7 s** is not comparable either way: that test and
the 974-document one run as two threads of one binary, so each is timed under the other's load.
...
**The four-hundred-and-thirty-ninth ran §2 whole, and one gate moved by exactly what the round was
for.** quorra 914/35/8/17 → **917/35/5/17**, `REFUSED` re-ratcheted 8 → 5 as `personwithdog.pdf`,
`bug1365930.pdf` and `issue12798_page1_reduced.pdf` left it — §11.4.7's four-component page is two
`Target::Readback` renders against one quorra device now, and the `differ` list is held to equality
by the gate and did not move. Tests 1604 → **1609** (five added: two in a new
`render-cpu/tests/four_component_page.rs`, one in `render-gpu`'s refusal suite, two in
`render-quorra`'s headless one); citations 6469 → **6490**; quotations 612 → **617**. **Every other
line is identical** — corpus 974 with 68 incomplete; oracle 1794 at 1690/104 with 905/68/786,
geometry 1 and 2, not comparable 14, no render 18 and the undiagnosed list empty; text 99.2%
(24003/24187) with 23 below the floor; PDFBox 99.8% (14257/14281) with 4 below; dates 1514/1545;
XMP 318/1 with 3191 properties; JPEG 2000 14 byte-identical; ledger 875 rows at 406/248/18/82/8/113
with 218 tables. **`clippy --workspace --all-targets` was not silent at `1c3e00a`** — one
`doc_markdown` warning on a word the round before added — which is worth one sentence because §2
lists that gate second; it is silent now. `doc/todo/00`'s step 7 is **not owed**: nothing under
`render-cpu/src`, `pdf-render` or `pdf-model` changed, so the CPU rasters the ambiguous bucket is
measured from are the same bytes, which the oracle's seven identical counts are the check on.
**Not run and therefore not claimed**: the fourteen fuzz targets, since no parser changed; `cargo
deny`, since no dependency moved; the window under `Xvfb`. ADR 0275.

**The four-hundred-and-thirty-eighth ran §2 whole after moving the quorra pin `2531f447` → `89d7dd77`, and one gate moved by exactly what the pin was taken for.** quorra 911/35/11/17 → **914/35/8/17**, `REFUSED` re-ratcheted 11 → 8 as `bug1755507.pdf`, `issue13520.pdf` and `issue18032.pdf` left it; tests 1603 → **1604**; citations 6460 → **6469**. **Every other line is identical** — corpus 974 with 68 incomplete, oracle 1794 at 1690/104 with 905/68/786, geometry 1 and 2, not comparable 14, no render 18 and the undiagnosed list empty; text 99.2% (24003/24187) with 23 below the floor; PDFBox 99.8% (14257/14281) with 4 below; dates 1514/1545; XMP 318/1 with 3191 properties; JPEG 2000 14 byte-identical; ledger 875 rows at 406/248/18/82/8/113 with 612 quotations and 218 tables. `cargo deny` is **run and clean**, because a dependency moved, and both cross-target checks build. `doc/todo/00`'s step 7 is **not owed**: nothing under `render-cpu`, `pdf-render` or `pdf-model` changed, so the CPU rasters the ambiguous bucket is measured from are the same bytes — which the oracle's seven identical verdict counts are the check on. **Not run and therefore not claimed**: the fourteen fuzz targets, since no parser changed; the window under `Xvfb`. ADR 0274.
...
**The four-hundred-and-thirty-second ran §2 whole, before and after, because it changes the
correctness oracle's rasteriser.** Tests 1568 → **1572**/11 skipped, citations 6364 → **6373**,
quotations 595 → **596**; corpus 974 with 68 incomplete unmoved; **the oracle's seven verdict counts
identical — 1794 at 1690/104 with 905/68/786, geometry 1 and 2, not comparable 14, no render 18, and
the undiagnosed list empty**; quorra 910/36/11/17 → **911/35**/11/17, one document leaving and
`DIFFERS_AT_THE_EDGES` re-ratcheted 24 → 23; text 99.2% (24003/24187) with 23 below the floor, dates
1514/1545, XMP 318/1 with 3191 properties, JPEG 2000 14 byte-identical, ledger 875 rows at
403/250/18/83/8/113 with 217 tables. **`doc/todo/00`'s step 7 was run before and after over all 786
ambiguous pages**: twenty names at or past −1 both times, the same twenty, every other entry moving
*up* and one moving down — `issue12295.pdf` −1.712 → −2.956, which is that page approaching a
geometry all five renderers overpaint. `tools/spec-errata` was run over the fourteen documents and
**no §10.7 quotation is stale**. **Not run and therefore not claimed**: the fourteen fuzz targets,
since no parser changed; `cargo deny`; the window under `Xvfb`. ADR 0268.

**The four-hundred-and-fortieth ran §2 whole, before and after, and moved four numbers — three of
them the ones a departing report is supposed to move.** It asked `doc/todo/23`'s standing row what
it contained before pricing it, and **77 of its 85 web documents and all three of the corpus's were
a group declared inside a soft mask** (ADR 0276): `build_soft_mask` clears the blending space in
force for a mask group's content — ADR 0220's finding, since §11.5.3 reduces such a group to one
luminosity — and the flag recording a *change* of space was outside that scope, so an isolated group
inside an `/SMask`'s group took the whole page off §11.4.7's ink route. Corpus 68 → **65**
incomplete; oracle 1690/104 → **1693/101** with **all seven verdict counts identical**; text
24003/24187 → **24007/24191** with 65 → **62** not gated, 99.2% and the 23 holding; tests 1609 →
**1611**, citations 6490 → **6505**, quotations 617 → **618** — **and the two conformance figures in the
table below were behind by a round**, 6469 and 612 against the 6490 and 617 session 439's own history
row records, which a before-run on the stashed tree is the only thing that shows; **quorra 917/35/5/17 name for name**,
which is the sharpest line here — it compares this backend's raster page by page and draws §11.4.7's
two rasters itself since ADR 0275, so three pages whose CPU raster changed still agree with it.
PDFBox 99.8% (14257/14281), dates 1514/1545, XMP 318/1, JPEG 2000 14, ledger 875 rows at
406/248/18/82/8/113 with 218 tables: unmoved. **`doc/todo/00`'s step 7 ran before and after over all
786 ambiguous pages and exactly two lines differ**, both of them this round's own pages —
`bug1703683_page2_reduced.pdf` +0.141 → +0.146 and `issue13520.pdf` +0.695 → +0.507, each losing its
`[incomplete]` label — with twenty names at or past −1, the same twenty, and the alarm holding for
the fifteenth consecutive run. **The oracle failed first and that is the gate working**: a page this
tree reports is excused a diagnosis, so two of the three arrived at its judgement with none, and
each now carries a ladder and a difference image in `AMBIGUOUS_PAGE_DRAWN_IN_INK`. **The web
population was re-surveyed whole**, 65 944 documents in 145 archives before and after: **905 → 851
incomplete**, 157 blending reports over 156 documents → **83 over 82**, §11.6.6's row 85 → **8** —
54 of the 77 become complete, 23 keep a report they already had, and three of those *join* the
non-separable row, which is why it rises 28 → **31** and is now the largest transparency row the
web has. `spec-errata check` was run over the fourteen documents and
**no §11.6 quotation is stale**. **Not run and therefore not claimed**: the fourteen fuzz targets,
since no parser or decoder changed; `cargo deny`, since no dependency moved; the window under
`Xvfb`, since nothing a person presses is involved. ADR 0276.

**The four-hundred-and-forty-first ran §2 whole, before and after, and moved two numbers — both of
them the conformance pair — while closing the largest transparency row the web had.** §11.3.5.3's
non-separable blend on a page composited in four components was reported by name at 31 of 65 944,
on the reason that "neither raster has a blend function that states" the rule that clause gives the
black component. **It is not a blend function** (ADR 0277): the clause's auxiliary functions
"operate on colours that are assumed to have red, green, and blue components", which is three,
which is what each of ADR 0262's two rasters holds — the chromatic one *is* the complements the
clause's first bullet asks for, and the black one is **neutral**, where `Sat` is 0, `SetSat(C, 0)`
takes the clause's own `else` arm and `SetLum` returns the neutral colour of the luminosity it is
given, so `Hue`, `Saturation` and `Color` come to `Cb` and `Luminosity` to `Cs`. That is the K rule
term for term, checked over 200 000 neutral pairs at a worst gap of **1.19 × 10⁻⁷** — one ulp, the
residue of `0.3 + 0.59 + 0.11` not being exactly 1 in binary floating point. **So the round deleted
a refusal and wrote no code**: no display-list member, no backend arm, no mapping. The explicit
route was written first and withdrawn with its own arithmetic recorded — `B(Cb, Cs) = Cb` under
§11.3.3 is Porter-Duff Destination-Over exactly, `tiny-skia` and `peniko` have that operator and
`quorra_scene::Compose` does not, so stating the rule rather than deriving it would have cost the
quorra backend all 31 documents for no pixel. Corpus **65** incomplete unmoved, because
`issue18032.pdf` loses its third report and keeps §11.4.4's and §11.4.6's; oracle 1693/101 with
**all seven verdict counts identical**; **quorra 917/35/5/17 name for name**, which is where the
identity stops being arithmetic and becomes two independent implementations — the black raster of
`issue18032.pdf` now carries a `Hue` and that backend agrees about it; tests 1611 → **1614**,
citations 6505 → **6529**, quotations 618 → **624**; text 99.2% (24007/24191) with 62 not gated,
PDFBox 99.8% (14257/14281), dates 1514/1545, XMP 318/1, JPEG 2000 14, ledger 875 rows at
406/248/18/82/8/113: unmoved. **`doc/todo/00`'s step 7 ran before and after over all 786 ambiguous
pages and every line is byte-identical**, twenty names at or past −1 and the same twenty, the alarm
holding for the sixteenth consecutive run — `issue18032.pdf` is not in that bucket, so its picture
was checked directly instead: **RMSE 22.86 of 65535** at 2×, ink 1.28942 → 1.29393, and the
difference image is empty everywhere except inside the one gradient rectangle the page draws.
**The web population was re-surveyed whole**, 65 944 documents in 145 archives before and after:
**851 → 824 incomplete**, 83 blending reports over 82 documents → **52 over 51**, that row 31 →
**0** — 27 of the 31 become complete, 4 keep §11.4.4's report, **none joins**, and §11.4.4's 135
over 129 and §11.4.6's 23 over 20 are identical document for document. `spec-errata check` was run
over the fourteen documents and **no §11.3.5 quotation is stale**. **Not run and therefore not
claimed**: the fourteen fuzz targets, since no parser or decoder changed; `cargo deny`, since no
dependency moved; the window under `Xvfb`, since nothing a person presses is involved. ADR 0277.

**The four-hundred-and-forty-second ran §2 whole and every page gate is unmoved, which is the
expected result and was checked rather than assumed.** It did `doc/todo/01`'s headline job — read
the `partial` rows nobody has re-read — with a reading list nothing here had used before: order the
rows by the commit that last wrote each `note = ` line, which `git blame` answers, and read from the
top. 40 of the 248 had notes older than commit 110 of 590; **32 were read against the code and the
clause, and fourteen were wrong**, with a fifteenth found beside them (ADR 0278). **§11.6's is the
longest-lived stale claim this project has recorded, 424 sessions**, past §9.3's 365. One row turned
into work — Table 20's `/EFF` was read by nothing while a host extracts an attachment's bytes, so a
document naming a cipher there and `/Identity` in `/StmF` handed the attachment back as ciphertext
in silence — and one into a labelled decision, §12.5.6.13's `/Path` outranking `/InkList`, which the
row called Table 185's rule and which Table 185 does not state. Corpus 974 with **65** incomplete,
oracle 1794 at **1693/101** with all seven verdict counts identical, **quorra 917/35/5/17 name for
name**, text 99.2% (24007/24191) with 62 not gated, PDFBox 99.8% (14257/14281), dates 1514/1545,
XMP 318/1, JPEG 2000 14: every one unmoved. Tests 1614 → **1616**, citations 6529 → **6543**,
quotations 624 → **628**, tables 218 → **219**, ledger 875 rows at 406/248/18/82/8/113 →
**410/244/18/82/8/113**. `spec-errata check` over the fourteen documents prints **151 and 27**,
unchanged for nine rounds, and **none of this round's own quotations lands on struck text**.
**`doc/todo/00`'s step 7 is not owed**: the one behavioural change is in `pdf-syntax`'s choice of
crypt filter for an embedded file stream, which no page reaches — every other edit in `crates/` is a
doc comment or a test — so the CPU rasters the ambiguous bucket is measured from are the same bytes,
which the oracle's seven identical verdict counts are the check on. **Not run and therefore not
claimed**: `cargo deny`, since no dependency moved; the window under `Xvfb`. **The fourteen fuzz
targets were not run either, and that is the one worth arguing**: `document` and `object` cover the
changed function, and what changed inside it is a `match` arm on a name that selects among methods
the same targets already reach — no new parsing, no new arithmetic, and the arm's failure mode is a
decryption that returns `None`, which is the path §7.6.6 already takes on every unauthenticated
stream. ADR 0278.

| gate | what it printed | where |
|---|---|---|
| tests | `1619 tests run: 1619 passed, 11 skipped` — the tenth skip is the four-hundred-and-seventh session's `the_fixed_bounds_against_the_references_own_spread`, which derives the oracle's own bounds and is run explicitly — and `cargo test --workspace --doc` **1 passed** beside it, so `cargo test --workspace` reports **1620**. The four-hundred-and-twelfth added **eight**: five in `pdf-model` for §12.7.5.4's two shapes of `/V` and Table 234's `/I`, and one apiece in `viewer-core`, `viewer-confined` and `viewer-qt` for the same clause read from a host. The four-hundred-and-thirteenth added **one**, in `viewer-ui`, for which of §14.8.2.5's two orders a copy takes. The four-hundred-and-fourteenth added **nine** for a document-wide search: three in `viewer-core`'s new `search` module, three in its headless harness, and one apiece in `select`, `viewer-ui`'s panel test and the fragment tests. The four-hundred-and-fifteenth added **two**, both in `pdf-model`'s `transparency_groups`: which space §11.6.6's inheritance puts in force, and what compositing in `/DeviceCMYK` costs against compositing on the device. The four-hundred-and-sixteenth added **two**, both in the new `spec-errata`: which of Table 172's `/RT` values makes an annotation a reply rather than a group member, and that a strikeout covering no glyph retires nothing. The four-hundred-and-seventeenth added **two**: that a marked-content section's associated files are named by §14.13.5's `/MCAF` since Errata Collection 3, and — in `spec-errata` — that the two extractions of one sentence disagree about where its spaces are, which is what had hidden 72 struck passages. The four-hundred-and-eighteenth added **one** — that a Type 3 glyph description finds the resources its own stream names, which is §7.8.3's first step since Errata Collection 3 — and the gate printed **1498**, two above what this line had been carrying. The four-hundred-and-nineteenth added **eight**, all in a new `pdf-model/tests/missing_resources.rs`: the two requirements Table 86 puts on `Do`'s operand, a form that states no `/Resources` against one that states an incomplete one, the same `Do` inside a hidden optional-content section and outside it, and `gs` and `scn` naming nothing — and the gate printed **1506**, so this line was not behind. The four-hundred-and-twentieth added **nine**: five in `viewer-core`'s new `readback` module for the bound and its eviction rule, two in its headless harness — that a second search answers what the first did without interpreting a page again, and that an edit forgets every page's readback — one in `pdf-model`'s `composite_fonts` for §9.4.4's vertical displacement, and one in `viewer-ui` asserting that `--trace`'s "every topic" mask is the topic list's own arithmetic — **and the gate printed 1515**, so this line was not behind for the second round running. The four-hundred-and-twenty-first added **ten**: four in the new `tools/pdf-retrieve` for a section's edges, its annotations and the readback it must not tidy, one in its `json` writer for the control characters a PDF's text contains, two in `pdf-model`'s new `retrieval` module for what a clause number is and how an address is matched, one in `pdf-model/tests/structure.rs` for the walk that used to see half of ISO 32000-2's tree, and two more inside the tool's own tests — **and the gate printed 1525**, so this line was not behind for the third. The four-hundred-and-twenty-second added **fourteen**: six in a new `pdf-model/tests/contents_entry.rs` for the four ways Table 31's `/Contents` reaches a blank page and which two of them are worth a word, and eight in the new `tools/safedocs` — an archive name becoming the URL its README specifies, one outside the set refused before the network, a ZIP member found through the tail and verified on the way out, a CRC that does not match, a body that is not a ZIP at all, a member name that is a path, a plan over the budget and the budget's own arithmetic — **and the gate printed 1539**, so this line was not behind for the fourth. The four-hundred-and-twenty-third added **three**, all in `pdf-model/tests/composite_fonts.rs` for §9.10.2's permission reaching an `Identity-H` font: a partial `/ToUnicode` completed by the program, an Okular signature appearance that reads back its signer's name, and a two-byte code that is *not* read as a character when nothing names it — **and the gate printed 1542**, so this line was not behind for the fifth. The four-hundred-and-twenty-fourth added **two**, both in `pdf-syntax`'s `document`: that eight threads reading one document all get the object — with a compile-time assertion that `Document: Send + Sync` beside it — and that the recursion guard is per thread rather than per document, which is the one hazard `RefCell` → `RwLock` could have introduced silently — **and the gate printed 1544**, so this line was not behind for the sixth. The four-hundred-and-twenty-fifth added **six**, all in a new `pdf-model/tests/hostile_functions.rs` for §7.10.4: a stitching function naming its own object, two of them naming each other, a chain of eight the bound admits, a k of 255 whose `/Encode` is read rather than defaulted, a k above the budget refused by count, and the size of `Function` the budget is priced against — **and the gate printed 1550**, so this line was not behind for the seventh. The four-hundred-and-twenty-sixth added **eight** for §11.4.7's page group drawn in the space it states: three in `pdf-render`'s new `blending` module for the conversion out of the ink cube, two in `pdf-model`'s `colour` — that the table a backend is handed is this crate's own conversion over 2401 points of the cube, and that §10.4.2.4 round-trips through §10.4.2.5 and not through the cube — and three in `transparency_groups` for a page composited in ink, a colour from outside its space, and §11.3.5.3's rule for the black component — **and the gate printed 1558**, so this line was not behind for the eighth. The four-hundred-and-twenty-seventh added **three** for §11.7.2's conversion *into* that space: one in `pdf-model`'s `colour` — that `rgb_to_ink` is a right inverse of the ink cube over 1296 colours of its own image, with §10.4.2.4's route put back beside it — and two in `transparency_groups`, for a `DeviceRGB` colour converted into ink and coming back, and for a document that names the press its `DeviceCMYK` is — **and the gate printed 1561**, so this line was not behind for the ninth. The four-hundred-and-twenty-ninth added **one**, in `pdf-model`'s `saving`, for the undo `doc/todo/01`'s fifth sweep found reachable from nothing at all — a field forgotten leaves the producer's own `/V` and logs no edit — **and the gate printed 1562**, so this line was not behind for the tenth. The four-hundred-and-thirtieth added **five** — three in a new `pdf-model/tests/dct_components.rs` for a three-component `DCTDecode` frame whose Adobe APP14 marker says transform 0, one marked transform 2 and the control with no marker at all, and two in `contents_entry.rs` for a `/Contents` part the file states is empty against one truncated to nothing — **and the gate printed 1567**, so this line was not behind for the eleventh round running. The four-hundred-and-thirty-first added **one**, in `tools/spec-errata`, for a quotation that lowers the sentence's first letter because it starts mid-sentence — nine words identical and one letter apart from §9.6.2.2's struck sentence, which the comparison could not see while it kept case — **and the gate printed 1568**, five above what this line had been carrying. The four-hundred-and-thirty-second added **four** — one in `pdf-render`'s `geom` for the smaller singular value checked against a sweep of directions, two in its `sub_pixel` for the substitute width under a similarity and under an anisotropic transform, and one in `render-quorra/tests/sub_pixel_coverage.rs` for a sub-pixel rule at seven angles on both backends — **and the gate printed 1572**, so this line was not behind. The four-hundred-and-thirty-third added **eight** — four in a new `pdf-model/tests/hostile_geometry.rs` for a fill, a stroke and a clip reaching ten million units and an ordinary square as the control, and four in `render-cpu`'s new `scan` module for the range the anti-aliased scan converter can express — **and the gate printed 1580**, so this line was not behind. The four-hundred-and-thirty-fourth added **four** — two in `silent_fonts.rs` for the two halves of the branch that counts a missing glyph, a code whose font contains an empty glyph for it against one that reaches `.notdef`, and two in `pdf-font` for the invariant a wider substitute must satisfy and for the range a dictionary declines to state — **and the gate printed 1584**, so this line was not behind. **The four-hundred-and-thirty-fifth added ten** — two in `pdf-render`'s `soft_mask` for the value a mask takes where its group's marks do not reach, over every kind of mask and both transfer cases; two in `render-cpu` for the premultiplied route that replaced `Pixmap::take_demultiplied`, one of which pins a fact about the *dependency* rather than about this tree; and six in a whole new `pdf-model/tests/hostile_budgets.rs` — one hostile fixture and one control apiece for the graphics-state stack and for tiling, a form that draws itself, and a four-million-operator stream, for the four interpreter bounds whose 83 documents nobody had opened until that round did (ADR 0271) — **and the gate printed 1594**, so this line was not behind for the second round running. **The four-hundred-and-thirty-sixth added five** — all in `transparency_groups`, for the three routes a document names its press by (§8.6.5.6, §14.11.5, §11.7.2), for §8.6.5.7's passthrough of a colour already in that press's space, for a grid that is the profile at its own samples and near it between them, for a colour converted into a named press coming back, and for the four components this tree still cannot sample — with one of the file's old tests replaced rather than added to — **and the gate printed 1599**, so this line was not behind for the third round running. **It was written as 1588 first and that was wrong**, because the number was copied from a run taken before the six were added: the precondition the four-hundred-and-eighteenth wrote down is not "a round that ran the gate" but a round that ran it *last*. **The four-hundred-and-thirty-seventh added four** — a §14.8.4.7.3 link element whose one object reference and sixteen `/QuadPoints` numbers are the clause's own EXAMPLE 1, Table 369's seven ruby and warichu types, a text state operator outside a text object surviving into the next one and a `Q`, and a renumbered clause taking the corrigendum's title in `tools/conformance` — **and the gate printed 1603**, so this line was not behind for the fourth round running. **The four-hundred-and-thirty-eighth added two and removed one** — `cpu_and_quorra_agree_on_a_non_isolated_group` replaces `quorra_refuses_a_non_isolated_group` on the same scene and asserts the pixel as well as the tolerance, and `quorra_refuses_a_non_isolated_group_that_blends` holds the refusal that is left — **and the gate printed 1604**, so this line was not behind for the fifth round running. **The four-hundred-and-thirty-ninth added five** — `a_four_component_page_composites_in_ink_and_converts_once` and `converting_before_compositing_is_a_different_page` in a new `render-cpu/tests/four_component_page.rs`, `the_gpu_refuses_a_four_component_page` in `render-gpu`'s headless suite, and `cpu_and_quorra_agree_on_a_four_component_page` and `quorra_will_not_take_the_pair_where_this_tree_would_hand_it_over` in `render-quorra`'s — **and the gate printed 1609**, so this line was not behind for the sixth round running. **The four-hundred-and-fortieth added two** — `a_group_declared_inside_a_soft_mask_leaves_the_pages_blending_space_alone` and `the_same_group_on_the_page_still_reports_and_still_draws_on_the_device` in `transparency_groups`, one fixture putting one isolated group in either of two places — **and the gate printed 1611**, so this line was not behind for the seventh round running. **The four-hundred-and-forty-first added four and removed one, a net three** — `a_non_separable_blend_takes_the_black_component_from_the_backdrop`, `the_luminosity_mode_takes_the_black_component_from_the_source` and `every_non_separable_mode_agrees_with_the_clause_over_four_components` in `transparency_groups`, all three held to §11.3.5.3's own pseudocode transcribed in that file and replacing the refusal test they make false, and `the_clauses_own_functions_give_the_black_components_rule_on_a_neutral_pair` in `render-cpu`'s `blend`, which is the identity that closed the row, over 200 000 pairs — **and the gate printed 1614**, so this line was not behind for the eighth round running. A fifth changed its name rather than being added: `variable_text`'s `a_stand_in_that_cannot_draw_the_value_declines` is `a_stand_in_draws_the_whole_value_or_none_of_it`, asserting the equivalence that holds on a machine with an Arabic face and on one without (ADR 0270). Where the five came from was not chased; the precondition is the one the four-hundred-and-eighteenth wrote down, that this line is current only for a round that ran the gate and copied its number. The four-hundred-and-forty-fourth added **three** for §10.7.4's clipping paragraph: two in `render-cpu`'s `scan` — a clip restated six times leaves the mask alone, and two boundaries in one pixel take the smaller coverage — and one in its `clip_bands`, which draws the same ladder rather than composing it. The **11** skipped is the four-hundred-and-twenty-third's own new `#[ignore]`d gate against `doc/corpora/pdfbox`. `clippy --workspace --all-targets` silent under `pedantic` + `unwrap_used`/`panic`/`arithmetic_side_effects`; `fmt --all --check` clean | `cargo nextest run --workspace`, **33.9 s** |
| corpus (974 pdf.js documents, page one) | `974 documents in 3.9s: 0 unopenable, 8 locked, 2 encrypted beyond us, 5 pageless, **65 incomplete**, 0 slow` — 65 until the four-hundred-and-fifteenth, which read §11.4.7's page-group `/CS`: one false report left and four silent departures arrived (ADR 0251, trap 5's kind of rise); **68 until the four-hundred-and-nineteenth**, which made a `Do`, a `gs` or an `scn` naming a resource the file never defines say so — `issue6541.pdf` and `issue8702.pdf`, both of them files whose producer asked for a mark the file cannot carry, and neither of them a page whose ink moves (ADR 0255). **Unmoved by the four-hundred-and-twenty-second**, which added a report for a `/Contents` naming an object the file does not define: no document in the 974 does that, and the witness came from `doc/corpora/pdf-differences` instead (ADR 0258). **69 since the four-hundred-and-twenty-sixth**, which draws §11.4.7's page group in the `DeviceCMYK` space it states — `personwithdog.pdf` left, and the other five keep the report with the reason it now names (ADR 0262). **68 since the four-hundred-and-twenty-seventh**, which gave §11.7.2 its conversion into that space: `issue12798_page1_reduced.pdf` left, three of the seven such documents are drawn in ink rather than one, and the four that remain report a different clause each (ADR 0263). **65 since the four-hundred-and-fortieth**, which found that what took the other four off that route was a *soft mask's* group being counted as a group the page composites in: `bug1703683_page2_reduced.pdf`, `bug1755507.pdf` and `issue13520.pdf` leave, 6 of the 7 are drawn in the space they state, and the two that reach the oracle's judgement for the first time needed a diagnosis apiece (ADR 0276). **Unmoved by the four-hundred-and-forty-first**, which drew §11.3.5.3's non-separable blend over four components: `issue18032.pdf` loses that report and keeps the two §11.4.4 and §11.4.6 give it, so the count holds while the page's ink moves (ADR 0277) | `tests/corpus.rs`, **5.0 s** |
| oracle (1794 pages vs poppler, mupdf, ghostscript) | `1794 pages (1693 we call complete, 101 incomplete)` — 1693/101 until the four-hundred-and-fifteenth, whose four new reports moved only that split, 1690/104 until the four-hundred-and-nineteenth, whose two did the same, and **1693/101 again since the four-hundred-and-fortieth**, whose three departing reports moved it back and nothing else; **905 agree / 862 of them complete**, **68 contradicted / 66 complete**, **786 ambiguous / 754 complete**, our geometry 1/0, reference geometry 2/2, not comparable 14/9, no render 18/0 — and **the undiagnosed ambiguous list printed empty**, which is the ratchet holding. Nothing moved in the four-hundred-and-sixth — it changed only what the gate *prints* about a contradicted page (ADR 0242) — nor in the four-hundred-and-seventh, which added a second `#[ignore]`d test to the same file that re-derives the bounds this gate judges by (ADR 0243) — and the run now ends with a second ranking, `contradicted, and furthest from the nearest reference`, headed by `bitmap-symbol-context-reuse.pdf` at 28.91 nearest. **And by the four-hundred-and-forty-third**, which regrouped three contradicted pages and moved no pixel: all seven counts identical and every one of the 1794 per-page lines byte-identical, with `CONTRADICTED_PAGE_ROUNDING` 2 → **0**, `CONTRADICTED_COINCIDENT_CLIP_EDGES` and `CONTRADICTED_IMAGE_SAMPLE_AT_THE_PIXEL_CENTRE` new at one apiece (ADR 0279). **Every number here is unmoved by the four-hundred-and-thirty-first**, which re-derived five pages of `CONTRADICTED_SUBSTITUTED_FONT` that had been admitted together and never opened and moved them to `CONTRADICTED_GLYPH_EDGES` — 17 → **12** and 21 → **26** — a group being a hypothesis this gate holds as one ratchet, deliberately, so that a hypothesis turning out wrong does not fail the build (ADR 0267). **And by the four-hundred-and-thirty-second**, which changed how the oracle's own rasteriser draws every sub-pixel stroke that is not axis-aligned: all seven counts identical before and after, which is what moving ink inside a page's own tolerance looks like (ADR 0268). **And by the four-hundred-and-fortieth**, whose three pages drawn in ink moved only the complete/incomplete split — and whose two *newly undiagnosed* ambiguous pages are the gate working: a page this tree reports is excused a diagnosis, so `bug1703683_page2_reduced.pdf` and `issue13520.pdf` arrived here with none and now carry `AMBIGUOUS_PAGE_DRAWN_IN_INK` (ADR 0276). **And by the four-hundred-and-forty-first**: all seven counts identical, and `issue18032.pdf` — the one page whose raster moved — is not among the 1794 this gate compares (ADR 0277) | `tests/oracle.rs`, **102.0 s** |
| text (vs `pdftotext`, same 974) | `overall 99.2% (24007/24191 words), 23 below 90%`, with 24 skipped and **62** incomplete and not gated — 62 until the four-hundred-and-fifteenth moved the corpus's own count and this line was not read, 65 until the four-hundred-and-nineteenth moved it again. Both the numerator and the denominator fell by the same 56 words, which is the two new documents leaving the gated set and is what that denominator is for. **23987 → 24003 and 25 → 23 in the four-hundred-and-twenty-third**, on §9.10.2's permission reaching an `Identity-H` composite font; `issue16553.pdf` and `javauninstall-7r.pdf` left the named list and nothing joined it. **24003 → 24007 of 24191 in the four-hundred-and-fortieth**, which is the three documents that stopped reporting joining the gated set: both numbers rise by four, 65 → **62** not gated, and 99.2% and the 23 hold | `tests/text_extraction.rs`, **31.0 s** |
| **text (vs PDFBox's frozen extraction, 40 documents)** | `40 documents in 0.4s: 0 skipped, 0 incomplete and not gated; overall **99.8% (14257/14281 words)** against PDFBox's stream order and 99.8% against its position-sorted output, **4 below 90%**` — new in the four-hundred-and-twenty-third, and the first run was 14254/14281 with 5 below, one of which was the defect above. The four are named in `PDFBOX_BELOW_FLOOR` with the reading beside each. **No new line in `doc/todo/02` §2**: it runs inside the `--ignored` invocation the row above already makes | `tests/text_extraction.rs`, **3.7 s** |
| **quorra vs the CPU oracle** (974 documents, page one, same display list) | `957 pages compared in 34.0s: **917 agree, 35 differ, 5 refused, 17 not comparable**` — 910/36 until the four-hundred-and-thirty-second, when `issue12810.pdf` left on ADR 0268 (34 787 sub-pixel strokes, 62.6% of their length within 40° to 50° of an axis, all of it ink the hairline was dropping and quorra was already drawing) — one refusal is `bug1721218_reduced.pdf`, whose coverage outgrows a 16384×16384 scratch image; **four are §11.4.6's stated shape**, which needs two Porter-Duff operators — `Compose` did not have them when this row was written and has had them since `89d7dd77` (ADR 0234, `QUORRA_FEEDBACK.md` section 14, and section 14.2 for why that was not enough); **four more were §11.4.4's non-isolated group** and **three of those left in the four-hundred-and-thirty-eighth**, which is the first time a name has left this list because the request under it was answered: `GroupSpec` gained Table 145's `/I` at quorra `89d7dd77`, `bug1755507.pdf`, `issue13520.pdf` and `issue18032.pdf` agree with the CPU oracle about the picture the clause states, and the fourth — `issue12798_page1_reduced.pdf` — stays refused with its *reason changed* to §11.4.7's four components, which no count could have shown (ADRs 0237 and 0274, section 16.1). Five of those eight were still refused after that round — the four stated shapes and `issue12798_page1_reduced.pdf` — and every one of the five used to *agree* about a picture both backends drew wrongly. **A tenth in the four-hundred-and-twenty-sixth**: `personwithdog.pdf`, whose page composites in `DeviceCMYK` and is therefore two rasters where a `Scene` renders one (ADR 0262, section 17) — and that one used to be *reported* rather than agreed. **An eleventh in the four-hundred-and-twenty-seventh**, `bug1365930.pdf`, which is the sharpest case section 17 has: it reports nothing and never did, because nothing on it composites, so the refusal replaces an agreement about a *correct* picture (ADR 0263). **All three of those left in the four-hundred-and-thirty-ninth**, which drew §11.4.7's two rasters through quorra — two `Target::Readback` renders against one device, which section 17.1 had established were possible and cheap — and they agree with the CPU oracle at 0.0288, 0.0093 and 0.0760 mean, page inks 20.3953/20.3659, 0.8637/0.8634 and 16.9057/16.9117. **The four §11.4.6 refusals stay and their reason changed**: the two operators section 14 asked for arrived, and the position this tree emits them from is one of the two quorra's builder refuses — with a group mark carrying no compositing operator at all, which is the second and larger obstacle (ADR 0275, section 14.2). **Identical name for name after the four-hundred-and-forty-first**, which is the round's sharpest line: `issue18032.pdf`'s black raster now carries §11.3.5.3's `Hue`, and this backend agrees with the oracle about what that returns on a neutral pair without being told anything new (ADR 0277) | `render-quorra/tests/corpus.rs`, **39.0 s** |
| dates | `1545 date strings in 974 documents: **1514 conform** to §7.9.4 (97.99%), 31 do not, over 22 distinct strings` | `tests/dates.rs`, **0.7 s** |
...
| **JPEG 2000 vs ISO/IEC 15444-5's reference software** | 30 corpus codestreams: **14 byte-identical, 13 differing, 3 not comparable**, and no remaining difference exceeds one level. `doc/JPEG2000_FEEDBACK.md` §§7–8 has the two defects behind that | `tests/jpeg2000.rs`, **11.8 s** |
| conformance | **6559 citations** (6554 until the four-hundred-and-forty-fourth composed a clip chain as a set intersection, 6543 until the four-hundred-and-forty-third read §8.5.4, §8.9.6.4, §9.6.2.2, §10.7.4 and §11.6.5.2 against three contradicted pages and wrote the subclause's clipping paragraph and its image paragraph's third witness into §10.7.4's row and into `oracle.rs`'s two new groups, 6529 until the four-hundred-and-forty-second read the ledger's oldest `partial` rows against the code and wrote §7.6.2's `/EFF` into `pdf-syntax`'s decryption and §12.5.6.13's `/Path` into `pdf-model`'s ink annotation, 6505 until the four-hundred-and-forty-first read §11.3.4, §11.3.5, §11.3.5.3, §11.4.7 and §11.3.3 against the pair of rasters and wrote the clause's own pseudocode into `pdf-model`'s transparency fixtures, 6490 until the four-hundred-and-fortieth read §11.5.3, §11.6.5.1, §11.6.6 and §11.7.2 against the scope a mask group's content runs in, 6460 until the four-hundred-and-thirty-eighth passed Table 145's `/I` through to quorra and wrote §11.4.4, §11.4.5, §11.4.6, §11.3.5.2 and §11.3.6 into `render-quorra`'s scene walk, its two group tests and four ledger rows, 6450 until the four-hundred-and-thirty-seventh read §14.8.4.7.3's link element against the code and rewrote §14.8.4.7.4 for Table 369's seven types, 6407 until the four-hundred-and-thirty-sixth read §8.6.5.5, §8.6.5.6, §8.6.5.7, §11.7.2 and §14.11.5 against the press a document names, 6399 until the four-hundred-and-thirty-fifth wrote ISO 32000-2 §C.1 and §C.2 into `content.rs`'s four resource bounds and into `tests/hostile_budgets.rs`, and §11.6.5.1 into `SoftMask::outside` — the first time any of those bounds cited a clause at all, 6378 until the four-hundred-and-thirty-fourth read §9.6.2.2, §9.6.5.2, §9.6.5.4, §9.7.4.2 and §9.10.2 against the substitution path and put ADR 0270's rule in all five, 6373 until the four-hundred-and-thirty-third wrote §7.3.3, §10.7 and §10.7.1 into `render-cpu`'s new `scan` module and its ledger rows, 6364 until the four-hundred-and-thirty-second wrote §10.7.4's turned rule into the code and the ledger, 6357 until the four-hundred-and-thirty-first read §9.5, §9.8 and §9.8.1 against the code and put the cap-height measurement in the last two, 6349 until the four-hundred-and-thirtieth's two fixes, 6317 until the four-hundred-and-twenty-seventh gave §11.7.2 its conversion, 6345 until the four-hundred-and-twenty-eighth's `page` fuzz target cited §7.3.10 — `SOURCE_ROOTS` is `crates`, `tools` **and `fuzz`** — and 6346 until the four-hundred-and-twenty-ninth's sweeps), all naming clauses the standard has; **631 quotations**, all verbatim — 630 until the four-hundred-and-forty-fourth quoted §8.5.4's effective-shape sentence in `render-cpu`'s `scan`, 628 until the four-hundred-and-forty-third quoted §10.7.4's clipping paragraph and its sampled-image rule, 624 until the four-hundred-and-forty-second quoted Table 20's `/EFF` and its `/StmF` fallback and Table 185's `/Path` operands, 618 until the four-hundred-and-forty-first quoted §11.3.5.3's two CMYK bullets and the sentence that says its functions take three components, in `pdf-render`'s `blending`, `render-cpu`'s `blend` and `test-scenes`, 617 until the four-hundred-and-fortieth quoted §11.5.3's opening pair of sentences beside the fixture that rests on them, 610 until the four-hundred-and-thirty-seventh quoted §14.8.4.7.3's two `shall`s about a wrapped span, 601 until the four-hundred-and-thirty-sixth quoted §14.11.5's Table 401 sentence about which direction a screen uses, §11.7.2's redefinition of `DeviceCMYK` inside a group, §8.6.5.5's substitution rule and §8.6.5.7's reason, 597 until the four-hundred-and-thirty-fifth quoted Table C.1's *Nested objects* row and its NOTE's figure of 28, which is the only depth ISO 32000-2 prints for a nested construct and is what settles `MAX_STATE_DEPTH` at nine times it, 596 until the four-hundred-and-thirty-third quoted §7.3.3's sentence handing a number's range to the implementation, 595 until the four-hundred-and-thirty-second quoted §10.7.4's area sentence, 592 until the four-hundred-and-thirtieth quoted §7.4.8 and §7.3.8.1/§7.3.8.2 for two fixes; **219** distinct tables cited by this tree and **253** named in the ledger's notes — **this figure said 218 from the four-hundred-and-twenty-fifth until the four-hundred-and-thirtieth ran the gate twice, before and after its own work, and read 217 both times**; where the difference went was not chased, and the rule it restates is that this table carries the gate's own number rather than the last round's arithmetic; **875 ledger rows** (**410** implemented, **244** partial, 18 reported, **82** inapplicable, 8 writer-side, 113 out-of-scope) — 406/248 until the four-hundred-and-forty-second read the 32 `partial` rows whose notes `git blame` put oldest and moved §8.9.3, §12.5.6.13, §14.11.2 and §14.11.2.1 (ADR 0278), 403/251 until the four-hundred-and-thirty-seventh's sweep round moved §9.3, §9.3.1 and §11.3.7.1, all three `partial` above notes that named nothing owed and two of them on a reason §9.3.8's own row had retired in the seventy-second session (ADR 0273), 403/250/18/83 until the four-hundred-and-thirty-sixth moved §8.6.5.7 off `inapplicable`, whose reason — that this device is an sRGB screen so there is no four-to-three-to-four round trip for the clause to save — stopped being true the moment a page composites in a press (ADR 0272), — 401/252 until the four-hundred-and-twenty-ninth's sweep round moved §9.4 and §9.7.5.1, both `partial` on reasons their own children's rows had retired, 401/251/19 until the four-hundred-and-twenty-seventh moved §11.7.2 off `reported`, and 400/252 until the four-hundred-and-twentieth, which read §9.4.4's row against the code and found it `partial` on one sentence that had been false since the thirty-sixth session: "[t]he vertical branch is not: ty is always 0, because nothing here reads a vertical writing mode", against §9.2.4's own row two clauses above saying "[b]oth writing modes, from the thirty-sixth session" (ADR 0256). **What that gate does not cover is now measured, and the four-hundred-and-sixteenth found a second thing it cannot see**: the 577 are every rustdoc blockquote in `crates/`, checked against a conversion that dropped every annotation in all fourteen documents — so **151 passages Errata Collection 3 struck out verify as the standard's current words** (ADR 0252, `tools/spec-errata`). That number was 79 until the four-hundred-and-seventeenth session read all 79 and found the checker itself blind to a space — both sides are extractions of the same glyphs by different programs, so one writes "inthe" where the other writes "in the" — and the 72 it had been missing included the sentence `crate::appearance` and `ledger.toml` were both quoting as §12.5.2's live rule (ADR 0253, `doc/errata-read.md`). And the ledger's own notes hold **977** quoted spans that nothing has ever checked, 417 of which occur in no document under `doc/md/`. ADR 0249 decides that as a sweep rather than a gate and says what a gate would cost; **the four-hundred-and-eighteenth ran that sweep against the errata** — which needs none of ADR 0249's syntax, because an erratum supplies the other side of the comparison — and found a *third* unchecked population beside it, quotation marks inside ordinary rustdoc prose, where five of this round's nine stale quotations were (ADR 0254). **The four-hundred-and-nineteenth found a fourth and a fifth**, both by walking into one of them while reading §7.8.3 for something else: a quotation inside an ordinary `//` comment, which the prose scanner skipped on a stated reason that `CLAUDE.md` contradicts, and a quotation with an ellipsis in it, which `overlaps` compared whole and so could never match a struck passage longer than itself. Twenty-one further landings, **six of them stale quotations in six files** — two of which are the two the round before recorded itself as having missed (ADR 0255) | `cargo test -p conformance`, **3.0 s** |
...
Read off that command in the four-hundred-and-forty-second and **re-read off it, unmoved, in the
four-hundred-and-forty-fifth**:
...
| `inapplicable` | 82 | a press, a layout engine, a production workflow — **and read at last** (ADR 0205) |
...
`partial` and `reported` are 262 rows between them and each names what it owes.
...
| A substitute that cannot be addressed; **5 codes over 2 documents that reach no glyph in silence** — `issue14821.pdf`'s three and `issue4650.pdf`'s two, read off the corpus gate's own line in the four-hundred-and-forty-fifth, where this row said **24 over 8** and had said it since before the four-hundred-and-thirty-fourth split that population (ADR 0270); beside them the gate prints **57 codes over 9 documents reaching a glyph the font draws blank**, which is not a mark missed; a per-character fallback, owed with no witness | 40 | [todo 21](todo/21-font-substitution.md) |
...
fallback rather than a reading. **The two halves of that sentence are two different refusals since
the four-hundred-and-thirty-fourth** (ADR 0270): a `loca` entry that is empty *is* the program's
statement that the code makes no mark, and `Interpretation::codes_reaching_a_blank_glyph` counts
it apart from a code that reaches nothing at all.
...
`tools/safedocs` has fetched — **65 944 documents over 145 archives and 93 GB since the four-hundred-and-thirty-third**, all of them surveyed — with each sample's own baseline and the licence of each), the 66
contradicted pages on complete documents grouped with their diagnoses, the **65** incomplete
documents split by report kind (this pointer said 70 for twenty-six rounds), and the two cautions the
contradicted list earned. Its **§3c** is the four-hundred-and-seventh session's
...
binaries, and `doc/todo/02-every-round.md` §5 is what puts the **six** release binaries and the one
library where a person can run or link them — `pdf-viewer`, `pdf-sandbox-worker`, `pdf-view-worker`,
`pdf-viewer-gtk`, `pdf-viewer-qt`, `pdf-retrieve` and `libviewer_ffi.so`. (This said *three* for
thirty-five rounds after the fourth arrived.)
...
`cargo deny`, the **five** cross-target checks under `-D warnings` — three for the two platforms
with no confinement and two more for the C ABI, which cross-compiles where the toolkit hosts
deliberately do not — the **fourteen** fuzz targets and which need a seeded corpus (five of them do),
the callgrind counters, the census and ladder examples, and the AT-SPI recipe. **This said "two"
while `doc/verify.md` listed five**, corrected in the four-hundred-and-forty-fifth, which ran all five.
