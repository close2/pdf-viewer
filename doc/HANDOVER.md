...
| **JPEG 2000 vs ISO/IEC 15444-5's reference software** | 30 corpus codestreams: **14 byte-identical, 13 differing, 3 not comparable** — and the 13 are `hayro-jpeg2000`'s irreversible path, written up in `doc/JPEG2000_FEEDBACK.md` and held by name so an upstream fix fails the build (ADR 0161). **The fix arrived in the three-hundred-and-eleventh session and the hypothesis was right**: 0.4.0 implemented *none* of ISO/IEC 15444-1 E.1.1.2's reconstruction bias, upstream `9cce046b` does, and this tree pins that revision because no release carries it (ADR 0190). The worst sample error over the corpus falls from **87 levels to 3** — `S2.pdf` object 33 from 102 139 samples wrong by up to 87 to **63 wrong by one** — and **not one codestream became identical**, so the list is still thirteen and the residual is an open question with the rounding mode already ruled out | `tests/jpeg2000.rs`, ~8 s |
