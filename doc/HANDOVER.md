...
Written 2026-07-26, updated 2026-07-31 at the end of the **ninety-first** working session. Read
...
**Eighty-ninth — which of five values applies.** §14.7.6 said which attribute objects are
attached to an element; §14.8.5 says which *value* applies, and adds two things the other clause
does not. ADR 0079.

- **An owner is not a label.** §14.8.5.2: it "determines the interpretation of the attributes
  defined in the object", so `/BBox` under `Layout` and `/BBox` under `HTML-4.01` are different
  attributes sharing a name. `structure::Owner` keeps the five PDF-native ones apart from the
  thirteen format-specific ones, which are one arm because this reader translates to none of
  those formats.
- **A format-specific owner is skipped rather than preferred**, which is the clause's own
  condition: its first priority applies "if processing based on the format indicated by the
  owner value".
- **Inheritance is a separate function on purpose.** §14.8.5.3's fourth priority walks `/P`, and
  whether an attribute inherits at all is stated per *attribute* in §14.8.5.4's tables — a reader
  that inherited everything would give a paragraph its table's `/ColSpan`.
- **§14.8.5.1 was `silent` while §14.7.6's rows recorded the same code as implemented**: the
  fourth understated row of this run of ten, and specific enough now to be a rule — when two
  clauses describe one mechanism, reviewing one of them leaves the other lying.

`silent` falls 97 → 93; clause 14 has seven rows left and none of them is a mechanism.

**Ninetieth — a ring of beads nobody writes.** §12.4.3's articles are a reading order laid over
pages that are not consecutive, and §12.6.4.7's thread action jumps to one. Both read, and the
sixth §12.6.4 action a click can perform. ADR 0080.

- **The chain is a ring**: `/N` "[i]n the last bead … shall refer to the first bead", so the walk
  cannot stop at an absent entry and the visited set is the termination condition of a *well
  formed* file rather than a defence against a broken one.
- **The standard states one array's order twice and the two disagree** — Table 31 says a page's
  `/B` is "in natural reading order", §12.4.3 says "in drawing order". So the check against the
  threads compares the *set*, which is the only comparison both sentences license.
- **No corpus document states an article**, measured over every object of all 974: two catalogs
  carry a `/Threads` entry (an empty array and a null) and no page carries a `/B`. Trap 8 at its
  purest — the clause's own EXAMPLE 2 is the test.
- **§12.4 and §12.4.1 were `silent` while two of their three children were not**, which is ADR
  0079's rule arriving one clause later.

`silent` falls 93 → 89.

**Ninety-first — the page as its producer drew it.** §12.3.4's thumbnails are an image
dictionary the clause *subtracts* from, and the miniature is the file's own picture of its page.
ADR 0081.

- **Only five of Table 87's entries are significant**, so a `/Thumb`'s `/SMask` reaches no pixel
  — and `/Filter` survives because it is **Table 5's** and the sentence names Table 87, which the
  clause's own EXAMPLE requires by writing one.
- **A thumbnail is a second statement about a page, made by the program that made the page.**
  All 11 corpus thumbnails decode; three differ from our render by more than 15 of 255 and the
  thumbnail is wrong in all three — one file pdf.js reduced, one 1×1, and one whose page all five
  renderers draw the same way.
- **A fourth name in that list is the interesting case**, which is why it is a ratchet.
- §12.3 and §12.3.1 had said "[n]ot one of them is implemented" since the forty-ninth session.
...
**§12.5.6.5's links, which a click follows** and which now perform **six of §12.6.4's actions** —
a layer turned off, an annotation hidden, a page turned, a URI resolved, an article thread
followed to its next bead — and **§14.9's four
...
page or runs a slide show — and the gap is now measured *by clause* as well as by corpus: 86 of
the ledger's rows are
`silent`, and almost every one of them is a viewer rather than a renderer.

- **768 tests**, `clippy` clean under `pedantic` + `unwrap_used`/`panic`/`arithmetic_side_effects`,
...
- **The citations are checked.** `tools/conformance` holds every `§` in the tree to a clause the
  standard has — 2157 of them — every rustdoc blockquote to the standard's own words, and the
...
cargo test -p conformance -- --nocapture   # 2157 citations, 206 quotations, 125 tables, 823 rows
...
| status | rows | |
|---|---|---|
| `implemented` | 330 | every normative requirement in the clause is executed |
| `partial` | 198 | some are; the note says which are not |
| **`silent`** | **86** | not implemented, and nothing says so |
...
**86 silences is the shape of the project**: it renders pages correctly and does very little
when a person clicks on one. **75** of them are clause 12's interactive half, **7** are clause
14's, **2** are §7.11's remaining file specifications and **2** are §9.8.3's substitution hints —
none of which changes a mark on a page. The count is one line of `grep` over the ledger grouped
by leading clause number, re-run in the ninety-first session; this file has twice carried numbers
taken before the rows that closed them.

**Twelve sessions took it from 178 to 86**, and almost none of that was rendering: §12.6's remaining
...
rows in the fifty-sixth session, so the specification track is now its **86 `silent` rows and 27
...
- **Clause 12's interactive half, 75 rows.** Nothing edits a field or shows a panel, and nothing
  ...
  **`SetOCGState` 1 in 1, `Launch` 1 in 1, and no `Hide` at all**; 32 documents carry an `/AA`.
  ...
  leaves `GoToE` at 31 actions in 2 documents as the largest — and **§7.11.4's embedded file
...
- **Clause 14's structure, 7 rows**, and what is left of them is small. §14.7 is read whole —
  ...
  strings are (ADR 0073), and §14.13's associated files are (ADR 0077). §14.8.5's mechanism is read as of the
  eighty-ninth (ADR 0079) while twelve of its sixteen rows stay `inapplicable`, because a layout
  attribute describes the process that produced an appearance this reader already has. The
  remaining silences are §14.8.5.6's `PrintField`, §14.8.2.5's content *ordering*, §14.8.2.6.2's
  word breaks, and §14.7.7's worked example. **The thing worth doing
...
and the debt is the 113 rows above — **86 `silent` and 27 `reported`, down from 201 twelve sessions ago**.
...
- **`doc/md/` is the specification, in a form code can read.** Markdown conversions of the 14
  ...
  **The conversion also drops content, which the seventieth session met for the first time**: Table
  ...
  where to expect this — the conversion reflows them.
