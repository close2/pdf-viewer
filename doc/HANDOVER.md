...
Written 2026-07-26, updated 2026-07-31 at the end of the **ninety-second** working session. Read
...
page or runs a slide show — and the gap is now measured *by clause* as well as by corpus: 77 of
the ledger's rows are
`silent`, and almost every one of them is a viewer rather than a renderer.

- **774 tests**, `clippy` clean under `pedantic` + `unwrap_used`/`panic`/`arithmetic_side_effects`,
...
| status | rows | |
|---|---|---|
| `implemented` | 334 | every normative requirement in the clause is executed |
| `partial` | 203 | some are; the note says which are not |
| **`silent`** | **77** | not implemented, and nothing says so |
...
**77 silences is the shape of the project**: it renders pages correctly and does very little
when a person clicks on one. **66** of them are clause 12's interactive half, **7** are clause
...
by leading clause number, re-run in the ninety-second session; this file has twice carried numbers
taken before the rows that closed them.

**Thirteen sessions took it from 178 to 77**, and almost none of that was rendering: §12.6's remaining
...
rows in the fifty-sixth session, so the specification track is now its **77 `silent` rows and 27
...
- **Clause 12's interactive half, 66 rows.** Nothing edits a field or shows a panel, and nothing
...
and the debt is the 104 rows above — **77 `silent` and 27 `reported`, down from 201 thirteen sessions ago**.
