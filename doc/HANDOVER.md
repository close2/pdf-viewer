...
| status | rows | |
|---|---|---|
| `implemented` | 399 | every normative requirement in the clause is executed |
...
a fact about the moment the command arrived and undo is a replay. What
[todo 33](todo/33-annotation-editing.md) still owes is a caret, and a host that sends the command
from a drag.
...
all 1794), and until the hundred-and-seventy-sixth session no gate watched one of them.** **42** are
still undiagnosed, from 754 — and the 72 this file said was `wc -l` of a file with a twelve-line
header, corrected in the three-hundred-and-seventeenth by counting what the gate counts: the twenty rounds from the two-hundred-and-fifty-first took three
...
cargo test -p conformance -- --nocapture   # 4475 citations, 465 quotations, 195 tables, 875 rows
