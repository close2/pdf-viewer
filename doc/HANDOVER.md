...
| oracle (1794 pages vs poppler, mupdf, ghostscript) | of **1682** we call complete: **856 agree**, **68 contradicted**, 749 ambiguous — **673 of them diagnosed and 76 held by name since the hundred-and-seventy-sixth** (§3a) — 9 not comparable, 2 a reference's geometry | `tests/oracle.rs`, **36 s** |
...
| **quorra vs the CPU oracle** (974 documents, page one, same display list) | **FAILING since session 310, diagnosed in the three-hundred-and-eleventh, and the fix is one line upstream**: six pages refused where the ratchet holds one, the five new ones all "over the stated budget" of 256 MiB — `encode.rs` charges the GPU coverage lane's `rgba16float` winding texture *unconditionally* while `device.rs` allocates it only `if !winding.is_empty()`, so the **default CPU lane** pays eight bytes per texel of its whole scratch sheet for a texture nothing creates. Reproduced with a patched local quorra: **913 agree, 43 differ, 1 refused**, 17 not comparable — this gate's exact pre-310 state. `doc/QUORRA_FEEDBACK.md` §10, [todo 12](todo/12-five-pages-quorra-newly-refuses.md). 28 of the 43 are the two rasterisers' glyph antialiasing and not a defect list (ADR 0156) | `render-quorra/tests/corpus.rs`, **27 s** |
...
**The 74 incomplete documents** — 76 until the two-hundred-and-eighty-second, where a `Tf` naming
...
session 156's `CMap`s took 15 off this list), 19 transparency, 10 annotations over 9 documents, 8 malformed images, 8 operator soup
...
all 1794), and until the hundred-and-seventy-sixth session no gate watched one of them.** 76 are
