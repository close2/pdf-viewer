...
read and walked by no control. **It draws a
...
| oracle (1794 pages vs poppler, mupdf, ghostscript) | of **1688** we call complete: **859 agree**, **67 contradicted**, 751 ambiguous — **all 786 diagnosed and 0 held by name, from 754 in the hundred-and-seventy-sixth** (§3a) — 9 not comparable, 2 a reference's geometry | `tests/oracle.rs`, **25 s** |
...
| **quorra vs the CPU oracle** (974 documents, page one, same display list) | **920 agree, 36 differ, 1 refused**, 17 not comparable — 27 of the 36 are the two rasterisers' glyph antialiasing and not a defect list (ADR 0156). It was 914/42 until the three-hundred-and-eighty-ninth, when §10.7.4's coverage rule reached a shape thinner than a device pixel on the *processor* (ADR 0226) and six documents stopped disagreeing about one — `bug1308536`, `issue11913`, `issue13447`, `160F-2019`, `issue7454` and `issue840` — with `issue16038.pdf` staying and moving **6.5359 to 1.8563**, the largest movement this gate has recorded. It was 913/43 until the three-hundred-and-sixty-eighth session, when `issue4260_reduced.pdf` left `DIFFERS_AT_THE_EDGES`: once a §10.7.4 mark is a whole device pixel row rather than a band at the shape's own fractional position, two rasterisers have nothing left to distribute differently (ADR 0208). It was **failing from session 310 to 311** with six refused: `encode.rs` charged the GPU coverage lane's `rgba16float` winding texture unconditionally while `device.rs` allocated it only `if !winding.is_empty()`, so the **default CPU lane** paid eight bytes per texel of its whole scratch sheet for a texture nothing created. Diagnosed, reproduced with a patched local quorra, reported as `doc/QUORRA_FEEDBACK.md` §10 and **answered upstream at `0a1ffb13` the same day** — `device_bytes` now answers zero for an empty sheet, which says the condition once instead of twice | `render-quorra/tests/corpus.rs`, **30 s** |
...
| conformance | **516 quotations**, all verbatim, **5139 citations**, **206** distinct tables cited by the tree, **875 ledger rows** — read off the gate in the three-hundred-and-ninetieth, which found this row saying 509/5095/208 while the tree at HEAD printed 516/5138/206: the three-hundred-and-eighty-ninth moved all three and recorded none. The *table* count went **down** by two, which is not explained here and is worth a minute from whoever next runs this gate: a table citation can leave the tree by being corrected as well as by being deleted | `-p conformance` |
...
| `reported` | 19 | not implemented, detected and named at runtime — **nine of §12.8.3's moved to `partial` in the three-hundred-and-seventy-seventh**, when the digest question turned out not to need the trust store the whole clause had been refused for (ADR 0215) |
...
cargo nextest run --workspace              # 1369 tests, 9 ignored. A user-local install:
...
cargo test --workspace --doc               # the one doctest nextest does not run: 1369 + 1 = 1370
cargo test -p conformance -- --nocapture   # 5195 citations, 520 quotations, 246 tables, 875 rows
