...
| oracle (1794 pages vs poppler, mupdf, ghostscript) | of **1685** we call complete: **856 agree**, **68 contradicted**, 750 ambiguous — **768 of the 786 diagnosed and 18 held by name since the hundred-and-seventy-sixth** (§3a) — 9 not comparable, 2 a reference's geometry | `tests/oracle.rs`, **36 s** |
...
| conformance | **429 quotations**, all verbatim, 4243 citations, 194 distinct tables cited by the tree, **823 ledger rows** | `-p conformance` |
...
| status | rows | |
|---|---|---|
| `implemented` | 388 | every normative requirement in the clause is executed |
...
**`silent` is one, and it was zero for three hundred and twenty-two sessions.** §10.5's transfer
function: `issue6931_reduced.pdf` states an `/ExtGState` `/TR` whose type-0 tables map 2/255 to
0.992, its image's every sample *is* 2, and the page's own text says *The color should be red*.
`poppler`, `ghostscript` and `hayro` apply the function and show a red heart on white; ours and
`mupdf` do not and show a black square; and nothing in this tree says a word. It came off
`doc/todo/00`'s **step 7** at +17.26, which is the instrument built for a page nobody is far from.
**Implementing it crosses `CLAUDE.md`'s own scope sentence** — "[h]alftones and transfer functions
describe a marking device; those are *inapplicable*" — so it is a question for the project owner
rather than a change to make, and [todo 13](todo/13-the-transfer-function.md) is the evidence.
Otherwise there is no requirement in the eight technical clauses that this program
