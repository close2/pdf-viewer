...
cargo test  --profile gates -p pdf-vfs        --test read_corpus    -- --ignored --nocapture   # RFC 0003 section 4: the whole layout listed, stat'd and read over the corpus, through the confined worker
cargo test  --profile gates -p pdf-vfs        --test awkward_classes -- --ignored --nocapture   # a document of each awkward class, from every corpus on the disk, through the confined worker
...
- **The `launch_path` line is `CLAUDE.md` principle 2's, and it is the one line here that is
  ...
  when warm; §5 already builds these binaries every fifth round.
