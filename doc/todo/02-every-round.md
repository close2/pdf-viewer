...
cargo nextest run --workspace               # 1394 tests, 9 ignored
...
- **`cargo nextest` is a user-local install** — `cargo install cargo-nextest --locked`, or the
  ...
  what CI runs. `nextest` skips doctests, which is why the line after it is there: 1394 + 1 = the
  **1395** `cargo test --workspace` reports. (This line said 1314 until the
