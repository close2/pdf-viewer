...
| [03](03-more-corpora.md) | **Three corpora added and the fetcher built** (ADR 0258): `pdf20examples`, `pdf-differences` and a sparse `pdfbox` under `doc/corpora/`, plus `tools/safedocs`, which takes a chunk of SafeDocs' 7 933 gigabyte archives through one byte range and refuses a plan over 32 MiB. What is left is *taking* a chunk a round, a licence reading for `openpreserve/format-corpus`, and two instruments the new files make possible. Promotion budget: 20 MB, and **0 MB of it is spent**, because a submodule is already reproducible | 974 + **108** |
