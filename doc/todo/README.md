...
| [65](65-the-remaining-frontier.md) | **The remaining frontier** — every `partial` and `reported` row grouped by why it is not done, for steering the campaign; `doc/HANDOVER.md` points at it for "what is left". |
| [66](66-the-mitigation-catalogue-build-out.md) | **The mitigation catalogue, site by site** — RFC 0007's mechanism is built and its bulk is not: a shipped profile's answers are carried out only at the sites with code behind them, and the converter says so per site. Grouped by the code one build unlocks. |
