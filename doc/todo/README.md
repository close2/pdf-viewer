...
| [22](22-variable-text-edges.md) | §12.7.4.3's remaining edges: a `/DA` font `/DR` lacks and cannot spell, a composite `/DA` font with no witness, a value a host cannot read back. The baseline's guard and the list box's cost both closed in the four-hundred-and-third (ADR 0240) | 1 |
...
| [30](30-a-native-host.md) | **Both native hosts are built** — GTK4 (`crates/viewer-gtk`, ADR 0244) and **Qt 6 through a C++ bridge** (`crates/viewer-qt`, ADR 0246) — each with a platform tree over three panel queries, real controls over `Query::Fields`, chrome in the platform's own colours, both host decisions and tier 1. **Neither added a message**, which was the condition on the C ABI, so **it may be frozen** with three amendments named first: `RasterFormat`'s `#[non_exhaustive]`, `Answer::Outline`'s borrow, and `Answer::Field`'s unreadable password. The second host also produced `crates/viewer-host` — four of `viewer-gtk`'s eight modules were never GTK's — and settled ADR 0245's scale question as a fact about platform controls rather than about one theme: every one of the 76 is taller than its `/Rect` on **both** toolkits | 102 of 964 documents have a delegable widget on page one |
