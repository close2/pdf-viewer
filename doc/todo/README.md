...
| [13](13-the-transfer-function.md) | §10.5's `/TR` — applied to solid colours, image samples and every colour a shading makes; what is left is §11.7.5.2's per-region model and a shading **pattern** whose transfer is resolved at the `scn` rather than at the mark, both reported and priced in the file. The other two clauses that were said to meet at that `scn` turned out to name *different* moments and are implemented (ADR 0483) |
