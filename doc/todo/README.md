...
| [37](37-a-frame-that-says-it-is-stale.md) | A frame that says it is stale — **built for the window with a graphics device** (ADR 0378): a slow view change shows its own pixels moved, says `approximated` in the trace, and is replaced by the real frame, with each of the five rules enforced by a test, a type or the structure. **Built for both windows** since ADR 0457, which gave the processor's the resample, rule 4 back with its argument, and a page's picture an identity that survives a re-interpretation. What is left is a render thread for that window and a placement per page in the presenter, neither a defect |
