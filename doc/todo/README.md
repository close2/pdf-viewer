...
| [54](54-what-quorras-answer-asked.md) | Five residues from reading quorra's own account of where it stands against this tree, three of them ours alone: type 4's comparison operators coercing a boolean where PostScript raises `typecheck` (a witness in `function.rs`, no corpus witness yet), a refusal list that flattens a device refusal and two of ours, two censuses that are one corpus walk, the fifth-frame tile-cache loss, and the two lanes' differing sets. Three of their asks were already discharged and are struck out with what discharged them |
