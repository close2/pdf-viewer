...
| [14](14-stream-the-decompression.md) | **Road D**, first of the three the owner ordered out of `10`: stream the decompression so a bomb never becomes an allocation — **a page's `/Contents` is read through a window and shipped** (ADR 0365), and what is left is the four nested content streams and a pump for the four filters that are not Flate |
...
| [37](37-a-frame-that-says-it-is-stale.md) | A frame that says it is stale — the owner asked for reprojection while a heavy frame is built, reluctantly, and the reluctance is in the specification: five rules keep an approximation off every path that judges a picture |
...
| [47](47-an-image-decoded-once-per-do.md) | The same image decoded once per `Do`: nine decodes of one photograph on one page, priced, with the three things a cache has to key on (ADR 0373) |
| [48](48-the-specification-we-check-against.md) | The specification we check against: `doc/md/` dropped every annotation and the annotations are the **errata** (ADRs 0252, 0253, 0254, 0255). The sixth population — every quotation of the standard in the Markdown this project wrote — **is read since the four-hundred-and-seventy-fourth** and its first run produced thirteen corrections (ADR 0309). **Items 1 to 3 closed in the five-hundred-and-fortieth** (ADR 0375): §8.9.5.4 implemented as the erratum states it and the reason it had been declined shown false, §14.8.6.3's enclosure requirement read and declined as a producer's `shall`, and the ledger's single-quoted spans read at last — by a rule shared with the other two populations, in a sweep that reads `ledger.toml` now. What is left: steps 4 and 5, the disagreement sweep and the substrate question |
