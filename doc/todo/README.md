...
| [11](11-shapes-that-still-disappear.md) | Shapes that still disappear: the sub-pixel fill, stroke, diagonal and the quantum's own boundary are closed (ADRs 0226, 0268, 0285) and §10.7.4's clip chain is half paid (ADR 0280). What is left is the same sentence one step along — `tiny-skia` multiplies the finished mask into the mark's own coverage — plus the cap a substitute does not draw under the quantum, and two marks abutting across a cell's box edge |
