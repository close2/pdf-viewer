...
| [30](30-a-native-host.md) | **GTK4 is built** (`crates/viewer-gtk`, ADR 0244): a real `GtkListView` over three panel queries, real controls over `Query::Fields`, chrome in the theme's own colour, both host decisions, tier 1 because GTK4 admits no other — and it needed **no new message**. **It absorbed `doc/todo/37` in the four-hundred-and-ninth**, whose one decision was taken (ADR 0245): the page is drawn without its widget appearances now, and what that measurement exposed is the *scale* — 11 of 76 controls wider than their `/Rect` and all 76 taller. Qt through `cxx-qt` is next, `viewer-ffi` last | 102 of 964 documents have a delegable widget on page one |
