...
| [47](47-search-performance.md) | A document-wide search is 6.19 s on 1023 pages — single-threaded by rule 4, and the cost is `interpret` at 5.7 ms a page. Four candidates, and memory or code quality may veto any of them | all |
...
| [49](49-restrictions-worth-re-examining.md) | Which restrictions are load-bearing and which are habit: the search is a pump for the hosts, `interpret` already uses rayon, and `Document`'s `!Sync` is the real blocker. The readback cache is decided by the owner's 10 MB bound | — |
