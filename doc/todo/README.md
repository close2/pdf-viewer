...
| [31](31-accessibility-host.md) | AccessKit over `Query::AccessibilityTree` — built and read back off a real AT-SPI bus (ADRs 0214, 0300), with a `TH`'s axis closed on one; what is left is `/Headers` and the search behind it, a place for an element that marks no text, a `Form`'s control role, the `Text` interface, actions, and what the question costs on a thousand-page document |
