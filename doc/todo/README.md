...
| [17](17-a-rebuild-that-misses-compressed-objects.md) | A rebuild that misses every compressed object: `xref::rebuild`'s scan finds `N G obj` headers, so a document whose cross-reference chain is unusable loses everything §7.5.7 packed into an object stream — in silence. The clause states the recovery itself and a strictly additive form of it cannot move a document that opens today; what is owed first is the population (ADR 0393) |
