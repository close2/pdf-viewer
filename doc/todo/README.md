...
| [10](10-bounds-that-cap-size.md) | Bounds that cap size rather than guard against a bomb — asked for by the project owner, with a witness they supplied: a 50 MB Bézier drawing this program can draw in a third of the fastest reference's time and truncates at 19% instead. Three defects owed on any road, and four roads — the fourth, streaming the decompression, being the only one that removes the allocation rather than surviving it |
