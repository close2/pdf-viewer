...
| [64](64-redaction-and-relocation.md) | **Redaction application and clause-driven relocation** — both built in batch seventeen (ADRs 1123, 1124 and the image cases); what stays is the design as *what is* and the two redaction cases still refused, a painted path or form and a JPX image. |
| [65](65-the-remaining-frontier.md) | **The remaining frontier** — every `partial` and `reported` row, and only those, grouped by why it is not done, for steering the campaign; membership re-derived from the live ledger and no tally carried. `doc/HANDOVER.md` points at it for "what is left". |
