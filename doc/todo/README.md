...
| [03](03-more-corpora.md) | **Three corpora added and the fetcher built** (ADR 0258): `pdf20examples`, `pdf-differences` and a sparse `pdfbox` under `doc/corpora/`, plus `tools/safedocs`, which takes a chunk of SafeDocs' 7 933 gigabyte archives through one byte range and refuses a plan over 32 MiB. **One of its two instruments is built** (ADR 0259): PDFBox's own frozen `*.pdf.txt` beside 40 of its PDFs, a second and undriftable reference for `text_extraction.rs`, whose first run found §9.10.2's permission declined for every `Identity-H` composite font. What is left is *taking* a chunk a round — `0000` and `3500` so far — a licence reading for `openpreserve/format-corpus`, and the oracle over the new corpora. Promotion budget: 20 MB, and **0 MB of it is spent**, because a submodule is already reproducible | 974 + **108** + 48 |
...
| [23](23-transparency-departures.md) | §11.4, §11.4.7 and §11.6.6, each reported where it can change a pixel — **§11.5.3's population is closed** (ADRs 0217, 0220), **§11.4.6's shape** (ADR 0234) and **§11.4.4's non-isolated group** (ADR 0237), leaving the blending colour space — read from §11.4.7's page group and §11.6.6's inheritance since ADR 0251, 7 documents, and priced at a four-component raster by that ADR's arithmetic — and, inside §11.4.6, a knockout group whose elements blend | 9 |
