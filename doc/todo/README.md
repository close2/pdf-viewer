...
| [64](64-redaction-and-relocation.md) | **Redaction application and clause-driven relocation** — both built, with the painted path, the form and three of `JPXDecode`'s four conditions cleared since (ADRs 1195, 1196, 1248). What stays is the design as *what is* and the cases a redaction still refuses by name: a stroke whose outline an expansion can only approximate, a codec image whose decode is off the grid its dictionary states, an inline image behind a codec, a Type 3 or non-`Identity-H` composite font, `sh`, and a soft-mask group. |
| [65](65-the-remaining-frontier.md) | **The remaining frontier** — every `partial` and `reported` row, and only those, grouped by why it is not done, for steering the campaign; membership re-derived from the live ledger and no tally carried, and **gated**, so a row that closes owes the map a deletion and a row that opens owes it a bucket (`cargo test -p conformance --test conformance the_frontier_map`, ADR 1250). `doc/HANDOVER.md` points at it for "what is left". |
| [66](66-the-mitigation-catalogue-build-out.md) | **The mitigation catalogue, site by site** — RFC 0007's mechanism is built and its bulk is not: a shipped profile's answers are carried out only at the sites with code behind them, and the converter says so per site. Grouped by the code one build unlocks; `tools/state.sh remedies` prints how many sites each target and each profile still leaves unbuilt. |
