...
| [03](03-more-corpora.md) | More corpora, the fetcher and the survey: three submodules under `doc/corpora/`, plus `tools/safedocs` over the SafeDocs crawl. What is left is *taking* a chunk a round and the oracle over the new corpora; `openpreserve/format-corpus`'s licence is **read**, and what remains of it is one question for the owner (ADRs 0258, 0261, 0264, 0266, 0269, 0271, 0302, 0303) |
| [10](10-bounds-that-cap-size.md) | Bounds that cap size rather than guard against a bomb — asked for by the project owner, with a witness they supplied: a 50 MB Bézier drawing this program can draw in a third of the fastest reference's time and truncates at 19% instead. Three defects owed on any road, and three roads |
| [11](11-shapes-that-still-disappear.md) | Shapes that still disappear: the sub-pixel fill, stroke, diagonal, the quantum's own boundary and the marks whose area is a *square* of the width — §8.4.3.3's caps and §8.5.3.2's dot — are closed (ADRs 0226, 0268, 0285, 0290), and §10.7.4's clip chain is half paid (ADR 0280). What is left is the same sentence one step along — `tiny-skia` multiplies the finished mask into the mark's own coverage — plus what an eight-bit raster does to a mark whose ink is under one of its levels, and two marks abutting across a cell's box edge |
...
| [31](31-accessibility-host.md) | AccessKit over `Query::AccessibilityTree` — built and read back off a real AT-SPI bus (ADRs 0214, 0300, 0301), with a `TH`'s axis and Table 379's `/BBox` closed on one; what is left is `/Headers` and the search behind it, the elements a `/BBox` does not reach, a `Form`'s control role, the `Text` interface, actions, and what the question costs on a thousand-page document |
...
| [33](33-annotation-editing.md) | Editing a free text annotation the *file* states — built, with Table 167's `LockedContents` read through the restriction policy (ADR 0304). What is left is Table 177's `/CL` callout line, which no corpus document states, and bit 8's `Locked`, which waits on deleting or moving an annotation |
