...
| [47](47-search-performance.md) | Half answered: a **repeated** search is 7.27 ms where it was 5.45 s, and 0.021 s in the window (ADR 0256). A **cold** one is unchanged at 4.79 s, and 19.6% of it is §7.7.3.2's page tree walked from the root once per page | all |
...
| [49](49-restrictions-worth-re-examining.md) | Which restrictions are load-bearing and which are habit: the search is a pump for the hosts, `interpret` already uses rayon, and `Document`'s `!Sync` is the real blocker. **Three of five settled** — the readback cache under a 4 MiB bound with `--cache-text` declined by measurement (ADR 0256), and item 5's structure-tree bound, which was hiding a quadratic walk: 16.8 s → 151 ms and 71 371 items → 129 389 (ADR 0257) | — |
