...
| [31](31-accessibility-host.md) | AccessKit over `Query::AccessibilityTree` — built and read back off a real AT-SPI bus (ADRs 0214, 0300, 0301, 0312), with a `TH`'s axis, Table 379's `/BBox`, a cell's `/Headers`, the empty answer a large tagged document's later pages got, a `Form`'s control role, the `Text` interface and what the question costs on a thousand-page document each closed on one (ADRs 0325, 0338, 0342, 0394), and **the actions a client may request** in ADR 0425; and the tree published per page on the screen rather than for the page showing in ADR 0445; what is left is the elements a `/BBox` does not reach, whether a stated `/BBox` should beat the shapes that were drawn, and four questions for the platform |
