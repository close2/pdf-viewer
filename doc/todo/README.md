...
| [30](30-a-native-host.md) | **GTK4 is built** (`crates/viewer-gtk`, ADR 0244): a real `GtkListView` over three panel queries, real controls over `Query::Fields`, chrome in the theme's own colour, both host decisions, tier 1 because GTK4 admits no other — and it needed **no new message**. Qt through `cxx-qt` is next, `viewer-ffi` last | — |
...
| [37](37-what-a-native-host-would-not-draw-itself.md) | **All six chrome populations cross as data** since ADR 0235, and a real native host is now built on them (ADR 0244). What is left is the page drawn *without* its widget appearances — **no longer a prediction**: 76 GTK controls over `160F-2019.pdf`'s 67 fields, each on top of the picture of itself, plus a second consequence the audit had not named, that a platform control's *minimum size* overflows a widget's own rectangle. Also `RadiosInUnison` carried and not obeyed | — |
