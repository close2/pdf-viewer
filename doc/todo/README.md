...
| [30](30-a-native-host.md) | GTK4, then Qt, then `viewer-ffi` — and since ADR 0235 such a host can build real controls over `Query::Fields` rather than take a form as pixels | — |
...
| [37](37-what-a-native-host-would-not-draw-itself.md) | **All six chrome populations cross as data** since ADR 0235 — `Query::Fields` carries §12.7.5's controls, Tables 227/229/231/233's flags, Table 234's items and the on-state name a check box needs, and the same round found that a check box could not be checked at all. What is left is the page drawn *without* its widget appearances, plus `RadiosInUnison` carried and not obeyed | — |
