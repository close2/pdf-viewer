...
//!
//! **Toolkit-free on purpose**, for the reason `panel.rs` gives: this is the decision, and
//! `src/controls.rs` is what builds the widget. The decision is the interesting half — it is
//! exactly the question `doc/todo/37` asked, "could a host that wanted a real `GtkEntry` build
//! one without reaching into `viewer-ui` or re-deriving anything", and the answer is this
//! function.
//!
//! `pdf_model::form`'s own module comment states the target: "[a] text field is a `QLineEdit`, an
//! `NSTextField`, a `GtkEntry`; a choice field is a combo box against a list". This is the third
...
///
/// One variant per control GTK4 has for the job, rather than one per §12.7.5 type: the clause's
...
pub enum ControlKind {
    /// §12.7.5.3's text field — a [`gtk4::Entry`], or a [`gtk4::TextView`] where Table 231 bit 13
    /// makes it multiline.
    Entry {
        ...
        /// the screen". GTK4 has a control *for* this, and it is not an entry with a mask.
        password: bool,
        /// Table 232's `/MaxLen`, which caps the control's own length.
        max_len: Option<u32>,
    },
    /// §12.7.5.2.3's check box — a [`gtk4::CheckButton`].
    Check {
        /// Whether it is on.
        on: bool,
    },
    /// §12.7.5.2.4's radio button — a [`gtk4::CheckButton`] in a group.
    Radio {
    ...
    /// §12.7.5.2.2's push button — a [`gtk4::Button`], with no value at all.
    Push,
    /// §12.7.5.4's combo box — a [`gtk4::DropDown`].
    Combo {
        ...
        /// list". GTK4's `DropDown` is not editable, so this is carried and reported rather than
        /// obeyed — see `controls.rs`.
        editable: bool,
    },
    /// §12.7.5.4's list box — a [`gtk4::ListView`] over its items.
    List {
