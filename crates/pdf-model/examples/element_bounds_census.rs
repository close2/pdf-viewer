...
//!
//! This counts three things, and the third is the one that decides whether reading the attribute
//! is worth anything:
//!
...
fn main() {
    ...
    for path in std::env::args().skip(1) {
        ...
        if counts.bounded > 0 || counts.malformed > 0 {
            println!(
                "{path}: {} element(s) with Table 379 /BBox, {} malformed, \
                 {} placeless of which {} bounded",
                counts.bounded, counts.malformed, counts.placeless, counts.placeless_bounded,
            );
        }
        total.absorb(&counts);
    }
