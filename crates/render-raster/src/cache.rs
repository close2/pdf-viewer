...
/// one unchanged page. raster keys every glyph-lane tile on that identifier, so every key went
...
pub(crate) struct ResourceCaches {
    outlines: HashMap<usize, Entry<Arc<Path>, raster_scene::OutlineId>>,
    /// Stroke outlines this crate expanded rather than uploaded as they stood ([`StrokeKey`]).
    strokes: HashMap<StrokeKey, Entry<Arc<Path>, raster_scene::OutlineId>>,
    images: HashMap<ImageKey, Entry<Arc<[u8]>, raster_scene::ImageId>>,
    ramps: HashMap<usize, Entry<Arc<ShadingKind>, raster_scene::RampId>>,
    ...
    /// when the last id naming those contents is released (raster's ADR 0053). So a program
    ...
    programs: HashMap<usize, Entry<Arc<[ProgramStep]>, raster_scene::FunctionId>>,
    ...
    stored: u32,
    /// What those uploads cost — the wall clock spent *inside* raster's own `upload_*` calls.
    ///
    ...
    /// per-segment upload, and an image's or a ramp's own size has raster's `bytes_uploaded`.
    segments: u64,
}
...
impl ResourceCaches {
    ...
    /// Records that `spent` was passed inside one of raster's `upload_*` calls.
    ...
    pub(crate) fn outline(&mut self, path: &Arc<Path>) -> Option<raster_scene::OutlineId> {
        let entry = self
            .outlines
    ...
    pub(crate) fn store_outline(&mut self, path: &Arc<Path>, id: raster_scene::OutlineId) {
    ...
    pub(crate) fn stroke(&mut self, key: StrokeKey) -> Option<raster_scene::OutlineId> {
    ...
    pub(crate) fn ramp(&mut self, kind: &Arc<ShadingKind>) -> Option<raster_scene::RampId> {
        let entry = self.ramps.get_mut(&key(Arc::as_ptr(kind).cast::<u8>()))?;
        entry.last_used = self.frame;
        Some(entry.id)
    }

    pub(crate) fn store_ramp(&mut self, kind: &Arc<ShadingKind>, id: raster_scene::RampId) {
        self.stored = self.stored.saturating_add(1);
        self.ramps.insert(
    ...
    ) -> Option<raster_scene::FunctionId> {
...
mod entries {
    ...
    use raster_scene::{ImageId, OutlineId, ResourceId};
