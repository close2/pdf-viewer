...
//! readback that dominates a `rasterize` call simply does not exist here — raster
...
//!
//! **Nothing here presents.** Since raster's ADR 0056 the surface leaves the device with
//! [`raster_gpu::Presenter`], and [`QuorraWindowRenderer::detach_presenter`] is the whole of what
...
/// carry the two usages a texture needs to be rendered into *and* sampled afterwards — raster's
...
pub struct WindowTextures<'a> {
    ...
    pub page: &'a raster_gpu::wgpu::Texture,
    ...
    pub chrome: &'a raster_gpu::wgpu::Texture,
}
...
/// been.** [`raster_gpu::Device::render`] measures its own three phases and already blocks on the
...
/// raster's own rule is that a wall clock is context and a timestamp query is evidence.
#[derive(Debug, Clone, Copy, Default)]
pub struct FrameCost {
    /// The whole call, which is what a host's own timer would have seen alone.
    pub total: std::time::Duration,
    /// Translating this frame's display lists into a [`raster_scene::Scene`] — the walk in
    /// [`build`], including every resource this crate uploaded on the way through.
    pub scene: std::time::Duration,
    /// Inside `scene`: the part of it spent *inside* raster's own `upload_*` calls.
    ///
    ...
    /// [`raster_gpu::Device::render`] end to end: the three below, plus acquiring the swapchain
    ...
    /// remainder a caller computes by subtracting the other three. A phase raster measures and a
    ...
    ///
    /// **An observable rather than an inference from a small [`Self::encode`]** (raster's ADR
    ...
    /// [`raster_gpu::Device::render_retained`] has no encode source to report, which is the
    /// same answer [`Self::device`]'s zero gives.
    pub encode_source: Option<raster_gpu::EncodeSource>,
    ...
    pub uploads: u32,
    /// Scene commands the device encoded, as raster counted them.
    pub commands: u32,
    /// Of those, how many reached no pixel of the target and were dropped before any geometry
    /// was built (raster's ADR 0015) — the share of a frame the window did not need.
    pub commands_culled: u32,
    /// Bytes raster scheduled for transfer to the device this frame.
    ...
    /// on a frame [`FrameSlot`] meant to replay** (raster's ADR 0050). ADR 0351 enumerated every
...
///
/// The floating-point fields compare by `PartialEq` rather than by bits, unlike the key raster
...
struct Retained {
    ...
    /// their cache entries with them, bump raster's resource generation, and cost every frame an
    /// encode — the reuse defeating itself through the cache it shares with the page.
    overlays: Vec<DisplayList>,
    scene: raster_gpu::RetainedScene,
...
///
/// **What this exists to stop paying for.** raster's `encode` walks the scene's commands,
...
/// first (`doc/todo/44` §3). `raster_gpu::RetainedScene` replays that encode when nothing an
...
///
/// - `Device::release` bumps a generation raster keys every retained encode on, so a frame that
...
impl FrameSlot {
    ...
    pub(crate) fn render(
        &mut self,
        device: &mut raster_gpu::Device,
        caches: &mut crate::cache::ResourceCaches,
        medium: Option<Medium>,
        frame: &PresentFrame<'_>,
        into: raster_gpu::Target<'_>,
        reported: &mut Reported<'_>,
    ) -> Result<raster_gpu::Frame, QuorraRasterError> {
        ...
        let mut release_error: Option<raster_gpu::DeviceError> = None;
        ...
        let held = match &mut self.held {
            ...
            slot => {
                ...
                caches.begin_frame();
                let mut builder = raster_scene::SceneBuilder::new();
                ...
                slot.insert(Retained {
                    ...
                    scene: raster_gpu::RetainedScene::new(builder.finish()),
                    transient,
                })
            }
        };
        ...
        // scene survive a zoom: same scene, new affine, raster re-encodes and nothing
        // here rebuilds or re-uploads anything.
        let viewport = if held.page_space {
            raster_gpu::Viewport::full(
                frame.width,
                frame.height,
                crate::scene::affine(
                    frame
            ...
            raster_gpu::Viewport::full(frame.width, frame.height, raster_scene::Affine::IDENTITY)
        };
...
impl FrameCost {
    /// Reads back what raster measured and counted, whichever way the frame was drawn.
    ///
    /// `phases` is filled rather than replaced, which is the whole reason it is an argument
    /// instead of a field: [`raster_gpu::Timings::phases`] is a `Vec` raster allocates every
    ...
    pub(crate) fn read(
        ...
        self.execute_measured = matches!(
            ...
            raster_gpu::TimingProvenance::TimestampQueries
        );
...
impl QuorraWindowRenderer {
    ...
    /// is roughly 80% of what bringing a device up blocks for (raster's ADR 0014). So a host
    ...
    #[must_use]
    pub fn instance_with(backends: raster_gpu::wgpu::Backends) -> raster_gpu::wgpu::Instance {
        raster_gpu::create_instance_with(backends)
    }

    /// The adapters *an instance* can see, which is the only list a host that restricted its
    /// backends may offer.
    ///
    /// `raster_gpu::Device::adapter_names` makes its own all-backends instance and so answers a
    ...
    #[must_use]
    pub fn adapters_on(instance: &raster_gpu::wgpu::Instance) -> Vec<String> {
        raster_gpu::Device::adapter_names_on(instance)
    }
    ...
    pub fn with_instance(
        instance: &raster_gpu::wgpu::Instance,
        window: impl Into<raster_gpu::wgpu::SurfaceTarget<'static>>,
    ) -> Result<Self, QuorraRasterError> {
        let device =
            raster_gpu::Device::for_surface_with_instance(instance, window, &crate::options())?;
        Ok(Self::around(device))
    }

    /// A presenter owning a surface on `window` — anything convertible to the
    /// re-exported [`raster_gpu::wgpu::SurfaceTarget`], which a winit window is.
    ///
    ...
    pub fn new(
        ...
        let device = raster_gpu::Device::for_surface(window, &crate::options())?;
        Ok(Self::around(device))
    }
    ...
    fn around(device: raster_gpu::Device) -> Self {
        ...
        device
            .wgpu()
            .0
            .on_uncaptured_error(Arc::new(move |error: raster_gpu::wgpu::Error| {
    ...
    /// What bringing the device up cost, in the three parts raster measures it in.
    ///
    ...
    #[must_use]
    pub fn startup(&self) -> raster_gpu::StartupTimings {
        self.device.startup()
    }
    ...
    /// The named spans raster measured inside the last frame, in raster's own words.
    ///
    ...
    /// whatever raster chose to name inside them, and what is in the list depends on what the
    ...
    ///   was built with `raster_gpu::Options::instrument_encode`. That is off by default and
    ...
    ///   [`FrameCost::device`] that are in none of the three durations, which is what raster's
    ///   own documentation of [`raster_gpu::Timings`] directs a caller to read the remainder
    ///   against.
    ///
    ...
    /// [`FrameSlot`] is already holding, and raster replays the device commands it made from
    ...
    /// up itself. [`Self::last_phases`] is the page's alone: raster names its spans with static
    ...
    pub fn render(
        ...
        let drawn = self.page.slot.render(
            ...
            raster_gpu::Target::Texture(into.page),
            &mut Reported {
                cost: &mut self.last,
        ...
        let chrome = self.chrome.slot.render(
            ...
            raster_gpu::Target::Texture(into.chrome),
            &mut Reported {
    ...
    pub fn render_page_only(
        ...
        let drawn = self.page.slot.render(
            ...
            raster_gpu::Target::Texture(into),
            &mut Reported {
                cost: &mut self.last,
                functions: &mut self.functions,
                phases: &mut self.phases,
    ...
    ///
    /// **Both usages, which is the trap raster's `LayerProblem::NotSampleable` exists to name**:
    ...
    #[must_use]
    pub fn layer_texture(&self, label: &str, width: u32, height: u32) -> raster_gpu::wgpu::Texture {
        let (gpu, _) = self.device.wgpu();
        gpu.create_texture(&raster_gpu::wgpu::TextureDescriptor {
            label: Some(label),
            size: raster_gpu::wgpu::Extent3d {
                width: width.max(1),
                height: height.max(1),
            ...
            mip_level_count: 1,
            sample_count: 1,
            dimension: raster_gpu::wgpu::TextureDimension::D2,
            format: raster_gpu::wgpu::TextureFormat::Rgba8Unorm,
            usage: raster_gpu::wgpu::TextureUsages::RENDER_ATTACHMENT
                | raster_gpu::wgpu::TextureUsages::TEXTURE_BINDING,
            view_formats: &[],
        })
    }
    ...
    #[must_use]
    pub fn medium_texture(&self) -> raster_gpu::wgpu::Texture {
        let (gpu, queue) = self.device.wgpu();
        let extent = raster_gpu::wgpu::Extent3d {
            width: 1,
            height: 1,
            depth_or_array_layers: 1,
        };
        let texture = gpu.create_texture(&raster_gpu::wgpu::TextureDescriptor {
            ...
            dimension: raster_gpu::wgpu::TextureDimension::D2,
            format: raster_gpu::wgpu::TextureFormat::Rgba8Unorm,
            usage: raster_gpu::wgpu::TextureUsages::TEXTURE_BINDING
                | raster_gpu::wgpu::TextureUsages::COPY_DST,
            view_formats: &[],
        });
        queue.write_texture(
            texture.as_image_copy(),
            &crate::scene::byte_colour(self.medium.surround),
            raster_gpu::wgpu::TexelCopyBufferLayout {
                offset: 0,
    ...
    /// that is not the one rendering (raster's ADR 0056).
    ///
    /// `None` for a device with no surface, and `None` on the second call. **It compiles nothing
    /// and waits for nothing** — raster's own documentation says it clones four handles and moves
    ...
    /// rules).
    #[must_use]
    pub fn detach_presenter(&mut self) -> Option<raster_gpu::Presenter> {
    ...
    /// # Errors
    ///
    /// [`raster_gpu::ForeignPresenter`] when the presenter came from another device — and it
    ...
    ) -> Result<(), raster_gpu::ForeignPresenter> {
...
pub(crate) fn build(
    ...
    caches: &mut crate::cache::ResourceCaches,
    medium: Option<Medium>,
    builder: &mut raster_scene::SceneBuilder,
    frame: &PresentFrame<'_>,
    ...
    if let Some(raster) = frame.raster {
        ...
        let spec = raster_scene::ImageSpec {
            width: raster.width,
...
fn fill(
    ...
    builder.rect(
        raster_scene::Rect::new(
            raster_scene::Point::new(left, top),
            raster_scene::Point::new(right, bottom),
        ),
        raster_scene::Affine::IDENTITY,
