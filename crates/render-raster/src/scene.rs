//! Display list → raster scene: the translation the two vocabularies were shaped
//! to make small.
//!
...
//! between two renders of one unchanged page, and raster keys its glyph atlas on that (ADR 0402).
...
pub(crate) struct Encoder<'a> {
    device: &'a mut raster_gpu::Device,
    list: &'a DisplayList,
    ...
    /// element replaces its backdrop within its own **shape**, which raster reads off the
...
pub(crate) enum ShadedPaint {
    /// A raster paint, ready to fill or stroke with.
    Ready(raster_scene::Paint),
    /// A sampled grid: not a raster paint — the caller draws it as an image
...
/// gives — or it is a raster clip chain.
#[derive(Debug, Clone, Copy)]
enum ResolvedClip {
    AdmitsNothing,
    Chain(raster_scene::ClipId),
}

/// What a command's clip admits: nothing at all (the command is skipped whole),
/// or a raster chain to draw under (`None` = unclipped).
#[derive(Debug, Clone, Copy)]
pub(crate) enum Admitted {
    Nothing,
    Chain(Option<raster_scene::ClipId>),
}
...
impl<'a> Encoder<'a> {
    ...
    /// One call across raster's boundary, with a clock on it.
    ///
    ...
    /// crate's walk of the display list, or raster's own upload. They have different owners, so
    ...
    /// raster's.
    fn handing_over<T>(&mut self, upload: impl FnOnce(&mut raster_gpu::Device) -> T) -> T {
    ...
    fn upload_outline(
        &mut self,
        outline: &[raster_scene::Segment],
    ) -> Result<raster_scene::OutlineId, raster_gpu::DeviceError> {
        ...
        for command in commands {
            match command {
                ...
                Command::Group {
                    ...
                    // `raster_scene::GroupSpec`'s, and the library multiplies the clip's
    ...
    /// [`raster_scene::Compose::Src`] cannot say it: that operator reads the shape off the
    ...
    /// ([`raster_scene::Compose::Plus`]). The two halves carry the same clip and the same
    ...
    fn shaped(
        ...
        self.stage(builder, shape, raster_scene::Compose::DestOut)?;
        self.stage(builder, object, raster_scene::Compose::Plus)
    }
    ...
    /// `raster_scene::GroupSpec::compose` is where raster took that ask (its ADR 0033).
    ///
    ...
    fn fill(
        ...
        // (ADR 0702's measured finding). Since raster's ADR 0086 the collapse table
        // is resident on the outline and raster's encode places the marks per
    ...
    fn emit_fill(
        &mut self,
        builder: &mut SceneBuilder,
        (outline, transform, rule): (raster_scene::OutlineId, Transform, FillRule),
        paint: &Paint,
        (clip, mask, blend): (
        ...
        // outline is not the shape raster is handed, and no corpus document strokes a cone.
        // ISO 32000-2 §8.7.4.3 Table 77's `/Background`: a colour the shading answers outside
        // its own bounds, which no raster shading kind expresses. §11.6.7 makes the wash and
        ...
        // through raster's mesh paint, which is precisely "an RGBA raster already at device
        ...
        let paint = match paint {
            ...
            Paint::Shading(shading) => match self.shading_paint(shading)? {
                ...
                // A sampled shading is not a raster paint: it draws as an image
    ...
    /// What a shading resolves to: a raster paint, the sampled kind (drawn as an
    ...
    pub(crate) fn shading_paint(
        ...
        let kind = match shading.kind.as_ref() {
            ShadingKind::Axial {
                start, end, extend, ..
            } => raster_scene::ShadingKind::Axial {
                start: point(*start),
                end: point(*end),
                extend: *extend,
            },
            ShadingKind::Radial {
                ...
                extend,
                ..
            } => raster_scene::ShadingKind::Radial {
                start: point(*start),
            ...
            ShadingKind::Mesh { triangles, ramp } => {
                return Ok(
                    match self.mesh(triangles, ramp.as_ref(), shading.transform)? {
                        Some(mesh) => ShadedPaint::Ready(raster_scene::Paint::Mesh(mesh)),
        ...
        Ok(ShadedPaint::Ready(raster_scene::Paint::Shading {
            ramp,
            kind,
            // raster anchors a shading through its own transform, exactly as the
    ...
    /// will take it — raster's `Paint::Function`, ADR 0376.
    ///
    ...
    /// 2. **Will the device execute it?** [`raster_gpu::function::admit`] — the structural
    ...
    /// at [`raster_gpu::function::MAX_PROGRAM_LENGTH`] steps — microseconds — against the
    ...
    fn function_paint(
        ...
        let steps: Vec<raster_scene::FnOp> =
            program.steps().iter().copied().map(function_op).collect();

        let analysis = match raster_gpu::function::admit(&steps) {
        ...
        let region = raster_scene::Rect::new(
            raster_scene::Point::new(x0, y0),
            raster_scene::Point::new(x1, y1),
        );
        ...
        let paint = raster_scene::Paint::Function {
        ...
        // The budgets the scene boundary states over a rectangle and a transform are raster's
        // own numbers, so they are asked with raster's own predicate rather than restated
    ...
    /// contents and drops it when the last id naming them is released (raster's ADR 0053), so
    ...
    fn function(
        &mut self,
        key: &Arc<[ProgramStep]>,
        steps: &[raster_scene::FnOp],
    ) -> Result<raster_scene::FunctionId, QuorraRasterError> {
    ...
    /// clipped to the filled path (integration note 9 in raster's plan).
    ///
    ...
    fn sampled_fill(
        ...
        // The grid's row 0 sits at the domain's y0; raster's image convention puts
        ...
        let grid_spec = raster_scene::ImageSpec {
            width: grid.width,
            height: grid.height,
        ...
        // Clip to the filled path: raster clips compose by chaining, so the fill's
    ...
    fn image(
        ...
        // Since raster's ADR 0089 (this tree's ADR 0706), an ordinary image crosses
        // the boundary as its own samples plus §8.9.5.3's flag, and raster's encode
    ...
    ///
    /// The raster is uploaded as a **mesh**, because raster's mesh paint is precisely "an
    ...
    ) -> Result<Option<raster_scene::MeshId>, QuorraRasterError> {
    ...
    pub(crate) fn outline(
        &mut self,
        path: &Arc<Path>,
    ) -> Result<raster_scene::OutlineId, QuorraRasterError> {
    ...
    fn cached_image(&mut self, image: &Image) -> Result<raster_scene::ImageId, QuorraRasterError> {
    ...
    fn ramp(&mut self, shading: &Shading) -> Result<raster_scene::RampId, QuorraRasterError> {
        ...
        let stops: Vec<raster_scene::Stop> = ramp
            .stops
            .iter()
            .map(|stop| raster_scene::Stop {
                offset: stop.at,
    ...
    /// The raster clip chain for a display-list clip, memoised per frame — `None`
    ...
    pub(crate) fn mask_id(
        ...
        builder: &mut SceneBuilder,
        mask: Option<SoftMaskId>,
    ) -> Result<Option<raster_scene::MaskId>, QuorraRasterError> {
        ...
        let kind = match def.kind {
            ...
            SoftMaskKind::Luminosity { backdrop } => raster_scene::MaskKind::Luminosity {
                backdrop: colour(backdrop),
            },
        };
        let transfer = def.transfer.as_ref().map(|t| {
            // The display list's table is private behind `apply`; raster's is the
            // table itself. Reconstructing through `apply` keeps the one source.
            raster_scene::Transfer(std::array::from_fn(|i| {
                #[expect(
                    clippy::cast_possible_truncation,
    ...
    /// baked into every command here, and the raster viewport stays identity —
    ...
    pub(crate) fn placed(&self, t: Transform) -> raster_scene::Affine {
...
/// The display list's transform, as raster's — same six coefficients, same
/// row-vector convention, same `then` order.
pub(crate) fn affine(t: Transform) -> raster_scene::Affine {
    raster_scene::Affine {
        a: t.a,
        b: t.b,
        c: t.c,
...
pub(crate) fn point(p: Point) -> raster_scene::Point {
    raster_scene::Point::new(p.x, p.y)
}

/// §7.10.1's `Range` as raster states it — the bounds, and the component count with them.
///
...
fn function_range(range: ProgramRange) -> raster_scene::FnRange {
    match range {
        ProgramRange::Gray(pair) => raster_scene::FnRange::Gray(pair),
        ProgramRange::Rgb(pairs) => raster_scene::FnRange::Rgb(pairs),
    }
}

/// One step of a compiled §7.10.5 program as raster's own instruction.
///
...
fn function_op(step: ProgramStep) -> raster_scene::FnOp {
    use raster_scene::FnOp;
    match step {
...
pub(crate) fn colour(c: Color) -> raster_scene::Color {
    raster_scene::Color::new(c.r, c.g, c.b, c.a)
}
...
pub(crate) fn fill_rule(rule: FillRule) -> raster_scene::FillRule {
    match rule {
        FillRule::NonZero => raster_scene::FillRule::NonZero,
        FillRule::EvenOdd => raster_scene::FillRule::EvenOdd,
    }
}
...
pub(crate) fn blend_mode(blend: BlendMode) -> raster_scene::BlendMode {
    match blend {
        ...
        BlendMode::Multiply => raster_scene::BlendMode::Multiply,
        BlendMode::Screen => raster_scene::BlendMode::Screen,
        BlendMode::Overlay => raster_scene::BlendMode::Overlay,
        BlendMode::Darken => raster_scene::BlendMode::Darken,
        BlendMode::Lighten => raster_scene::BlendMode::Lighten,
        BlendMode::ColorDodge => raster_scene::BlendMode::ColorDodge,
        BlendMode::ColorBurn => raster_scene::BlendMode::ColorBurn,
        BlendMode::HardLight => raster_scene::BlendMode::HardLight,
        BlendMode::SoftLight => raster_scene::BlendMode::SoftLight,
        BlendMode::Difference => raster_scene::BlendMode::Difference,
        BlendMode::Exclusion => raster_scene::BlendMode::Exclusion,
        BlendMode::Hue => raster_scene::BlendMode::Hue,
...
/// is positional.
pub(crate) fn segments(path: &Path) -> Vec<raster_scene::Segment> {
    path.commands()
        .iter()
        .map(|command| match *command {
            PathCommand::MoveTo(p) => raster_scene::Segment::MoveTo(point(p)),
            PathCommand::LineTo(p) => raster_scene::Segment::LineTo(point(p)),
            PathCommand::CurveTo(c1, c2, to) => raster_scene::Segment::CubicTo {
                c1: point(c1),
                c2: point(c2),
                to: point(to),
            },
            PathCommand::Close => raster_scene::Segment::Close,
        })
        .collect()
}

/// A display-list image as raster's upload spec — the same layout (straight-alpha
/// RGBA8, top row first, no padding), so the `Arc` is shared, not copied.
fn spec(image: &Image) -> raster_scene::ImageSpec {
    raster_scene::ImageSpec {
        width: image.width,
        height: image.height,
