//! The raster backend: [`Rasterizer`] over the document renderer this viewer's
...
//! side: the display list's vocabulary maps onto raster's scene almost one-to-one —
...
//!
//! - **Shadings.** Axial and radial map directly (raster anchors a shading through
...
//! - **Strokes.** raster strokes but does not dash — dashing is settled on this
...
//! - **Alpha and the medium.** raster renders onto transparency and hands back
...
pub enum QuorraRasterError {
    ...
    Render(#[from] raster_gpu::RenderError),
    /// raster's scene boundary refused a value the display list carried.
    #[error("scene refused: {0}")]
    Scene(#[from] raster_scene::SceneError),
    ...
    /// shared variant rather than a raster-specific one so that a host reports one refusal by
...
/// that chooses it: every constructor below and every caller that wants raster's defaults plus
/// this decision spreads `..render_raster::options()` rather than repeating the reasoning.
///
...
/// five round-robin rounds, raster's `encode` phase falls from 467 ms on one thread to 221 on
...
///
/// **Nothing here is built at startup.** raster clamps this number when the device is
...
#[must_use]
pub fn options() -> raster_gpu::Options {
    raster_gpu::Options {
        encode_threads: std::thread::available_parallelism().map_or(1, std::num::NonZeroUsize::get),
        ..raster_gpu::Options::default()
    }
}

/// The raster backend: a persistent device, resource caches keyed by the display
...
impl QuorraRasterizer {
    /// A backend on the best adapter raster can find.
    ///
    /// Returns as soon as the device exists; raster compiles its warm pipeline set
    ...
    pub fn new_headless_software() -> Result<Self, QuorraRasterError> {
        Self::with_options(&raster_gpu::Options {
            adapter: Some("llvmpipe".into()),
            ..options()
        })
    }

    /// A backend with explicit raster options — `RENDER_LIBRARY.md` section 4.5 makes the glyph
    /// cache's sub-pixel quantum the caller's decision to take, and this is where
    /// a caller takes it.
    ///
    /// **Spread [`options`] rather than [`raster_gpu::Options::default`]** unless the point is to
    /// have raster's own defaults: the difference is `encode_threads`, whose default is 1 and
    ...
    /// The adapter raster selected, for reports and golden-file metadata.
    #[must_use]
    ...
    /// swapchain, and raster's own survival table says the target is not part of the question.
    ...
    /// The named spans raster measured inside the last frame — see
    ...
    /// than the tiles it replaced (raster's ADR 0049).
    ///
    /// **Deliberately not a [`FrameCost`] field**, which is the argument
    /// `doc/QUORRA_FEEDBACK.md` section 25.3 made to the raster team and this keeps: the pair answers a question about a *corpus*
...
impl Rasterizer for QuorraRasterizer {
    ...
    fn rasterize(&mut self, list: &DisplayList, target: TargetSpec) -> Result<Raster, Self::Error> {
        ...
        // and costs no geometry at all is `raster-gpu/tests/two_rasters.rs`'s claim, and
        ...
        // medium in the space where that composite is exact. raster hands back straight
...
impl QuorraRasterizer {
    ...
    fn render(
        ...
        let rendered = built.and_then(|()| {
            ...
            let viewport = raster_gpu::Viewport::full(
                target.width,
                target.height,
