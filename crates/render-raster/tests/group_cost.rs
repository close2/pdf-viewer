...
//! `render-raster` is the only crate in this workspace that dev-depends on the other two, so
...
/// raster, which needs a device — the software one, for the reason `headless_quorra.rs`
/// gives: a suite that skips reports success while verifying nothing.
#[test]
fn the_quorra_backend_refuses_a_page_past_the_bound() {
    let mut raster = match render_raster::QuorraRasterizer::new_headless_software() {
        Ok(rasterizer) => rasterizer,
        Err(error) => panic!(
            "no software adapter for raster: {error}\n\
    ...
    let Err(error) = raster.rasterize(&list, target) else {
        panic!("raster drew a page past the group-blit bound");
    };
    names_the_bound(&error.to_string());
    assert!(
        matches!(
            error,
            render_raster::QuorraRasterError::Target(BackendError::GroupsTooCostly { .. })
        ),
        "raster refused for another reason: {error}"
    );
}
