//! The raster backend, verified headlessly against the CPU oracle.
//!
...
/// Builds the raster backend, or explains why the whole suite should fail.
...
fn compare_at(what: &str, list: &pdf_render::DisplayList, scale: f32) -> Comparison {
    ...
    let ours = raster()
        .rasterize(list, target)
        .unwrap_or_else(|e| panic!("{what}: raster refused: {e}"));
...
/// to produce. That is raster's stated unorm rounding — 0.77 of 255 in its ADR 0033 — and
...
fn cpu_and_quorra_agree_on_a_knockout_element_that_states_its_shape() {
    ...
    let ours = raster()
        .rasterize(&list, target)
        .expect("raster draws §11.4.6's two stages");
...
/// test in this tree as well as in raster's.
#[test]
fn quorra_states_what_it_will_not_stage() {
    let staged = |blend, isolated| raster_scene::GroupSpec {
        alpha: 1.0,
        blend,
        clip: None,
        knockout: false,
        mask: None,
        compose: raster_scene::Compose::DestOut,
        isolated,
    };
    let refusal = raster_scene::SceneBuilder::new()
        .group(staged(raster_scene::BlendMode::Multiply, true), |_| Ok(()))
        .expect_err("a blend mode composites the group by §11.3.5");
    assert!(
        matches!(
            refusal,
            raster_scene::SceneError::GroupComposeUnsupported {
                reason: raster_scene::GroupComposeReason::BlendNotNormal,
                ..
            }
        ),
    ...
    let refusal = raster_scene::SceneBuilder::new()
        .group(staged(raster_scene::BlendMode::Normal, false), |_| Ok(()))
        .expect_err("§11.4.4's backdrop would arrive inside the shape");
    assert!(
        matches!(
            refusal,
            raster_scene::SceneError::GroupComposeUnsupported {
                reason: raster_scene::GroupComposeReason::NonIsolated,
                ..
            }
        ),
        "the vocabulary names what it will not take and why: {refusal:?}"
...
fn cpu_and_quorra_agree_on_a_non_isolated_group() {
    ...
    let ours = raster()
        .rasterize(&list, target)
        .expect("raster draws a non-isolated group whose composite is Normal");
...
/// and a scratch per element. `GroupSpec` carries both of Table 145's flags, but raster's
...
fn cpu_and_quorra_agree_on_a_four_component_page() {
    ...
    let ours = raster()
        .rasterize(&list, target)
        .expect("raster draws a page whose blending space has four components");
    let at = |x: u32, y: u32| {
...
///
/// `test_scenes::radial_cone` has the geometry. raster has its own radial primitive, and it is
...
/// raster through its mesh paint, which is exactly "an RGBA raster already at device
...
/// draw differently — the CPU stretches the grid as a padded `tiny-skia` pattern, raster
...
/// reassociate and fuse (WGSL 15.7.5) is what made raster withdraw an exactness claim over it. So the
...
/// carries no inexact operator's value, so raster's `Agreement::Bounded` holds and the
...
/// raster's `Agreement::Unbounded` refuses the program at the upload — while a scene still
