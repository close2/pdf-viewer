...
//! - `render-raster` needs no such inverse, and had the same defect one step along: it resolves a
...
//! `paint_space` guard out of `render-raster/src/stroke.rs`, and the `expect`s below fail on the
...
fn backends() -> Vec<Backend> {
    ...
    match QuorraRasterizer::new_headless() {
        Ok(mut raster) => backends.push((
            "raster",
            Box::new(move |list, target| raster.rasterize(list, target).expect("a raster raster")),
        )),
        Err(why) => println!("  omitted: raster has no device here ({why})"),
    }
