...
//! `kurbo` and raster draw the spike. A fix in the backend that was wrong is only half of the
...
//! that dropped it.
//!
//! `render-raster/examples/mitre_ladder` is the instrument to run when this fails: it prints what
...
/// and both alphas, the worst single figure being raster's −0.11% at scale 2.5. An eight-bit
...
fn backends() -> Vec<Backend> {
    ...
    let mut raster = QuorraRasterizer::new_headless().expect("a raster device");
    ...
    vec![
        ...
        (
            "raster",
            Box::new(move |list, target| raster.rasterize(list, target).expect("a raster raster")),
        ),
        (
            "vello",
