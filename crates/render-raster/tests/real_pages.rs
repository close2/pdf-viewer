//! Real specification pages through the raster backend, against the CPU oracle.
//!
...
//!
//! - **Fidelity**, with the glyph-phase quantum off: what the adapter and raster
...
//!
//! **The second gate was the only instrument in either tree that could see raster's ADR
...
/// a worst observed of mean 1.85 / worst tile 20.2 / ssim 0.9670; raster's ADR 0073
...
fn the_glyph_quantum_cost_stays_bounded() {
    let mut raster = QuorraRasterizer::new_headless()
        .unwrap_or_else(|e| panic!("no adapter available for raster: {e}"));
    for (name, index) in CASES {
        let list = interpret(name, index);
        for scale in [1.0_f32, 1.9008] {
