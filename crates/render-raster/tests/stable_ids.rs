...
/// space instead of handing raster a scalar width. An isotropic placement takes the other branch
...
fn a_page_that_has_not_changed_uploads_nothing_the_second_time() {
    ...
    assert_eq!(
        ...
        0,
        "an unchanged display list re-uploaded a resource, so the identifiers raster keys its \
