...
//! ZOOM_FRAME_ROUNDS=5        cargo run --release -p render-raster --example zoom_frame -- …
...
//! ```
//!
//! **`ZOOM_FRAME_COVERAGE` names the coverage lane, and `cpu` — raster's own default — is what
...
//! moved view on a real adapter (this tree's ADR 0700, raster's 0080/0081), so a zoom or drag step
...
//! raster says it threw every tile placement away after it. Default: `1,<factor>`, which is the
...
//! translation spent *inside* raster's own `upload_*` calls, so `scene − handed` is this crate's
...
//!
//! **`bytes_uploaded` is printed because nothing else in this tree prints it.** raster counts the
...
//!
//! **`ZOOM_FRAME_ENCODE_PHASES` costs what raster says it costs.** `Options::instrument_encode`
...
use render_raster::{FrameCost, PresentFrame};
...
struct Sample {
    ...
    commands: u32,
    culled: u32,
    /// `" replayed"` or `" record-replayed"` after raster's own word for where the encode
...
impl Sample {
    fn of(cost: FrameCost, phases: &[(&'static str, std::time::Duration)]) -> Self {
        // `device` minus the phases raster names — the same arithmetic the viewer's own summary
        ...
        Self {
            ...
            replayed: match cost.encode_source {
                ...
                Some(raster_gpu::EncodeSource::RecordReplayed) => " record-replayed",
                Some(raster_gpu::EncodeSource::Encoded) | None => "",
            },
...
fn main() {
    ...
    // thread raster's geometry phase spawns, so an attribution of the walk this example is
    ...
    // The lane, defaulting to raster's own default so every earlier table stays comparable;
    ...
    let coverage = match variable("ZOOM_FRAME_COVERAGE").as_deref() {
        None | Some("cpu") => raster_gpu::Coverage::Cpu,
        Some("gpu") => raster_gpu::Coverage::Gpu,
        Some("compute") => raster_gpu::Coverage::Compute,
    ...
    for _ in 0..rounds {
        let mut backend = render_raster::QuorraRasterizer::with_options(&raster_gpu::Options {
