...
//!
//! raster divides the coverage rasterisation of one frame across
...
//!
//! ```sh
//! cargo run --release -p render-raster --example encode_threads -- <file.pdf> [page] [scale] [1,2,4,…]
//! ENCODE_THREADS_ROUNDS=7 cargo run --release -p render-raster --example encode_threads -- …
//! ```
//!
//! **A cold device per sample**, which is the one thing that makes this measurement honest:
//! raster's tile cache answers a second frame at the same transform from the atlas, so a
...
use render_raster::PresentFrame;
