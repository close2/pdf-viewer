...
//! `render-raster/tests/mitre_limit.rs` fails, and read three things:
//!
...
fn main() {
    ...
    let mut raster = render_raster::QuorraRasterizer::new_headless().expect("a raster device");
    let mut gpu = render_gpu::GpuRasterizer::new_headless().expect("a vello device");
    println!("raster adapter: {}", raster.adapter_description());
    ...
    for phi_deg in [
        ...
        let each = [
            ...
            raster.rasterize(&list, target).expect("a raster raster"),
        ];
