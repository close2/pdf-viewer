...
//!
//! Both were asked for by the raster team and are recorded as ours in
...
//!
//! - **The rectangular fills.** raster's ADR 0047 gives a rectangle its own lane, and section 19
...
//!   its own rasterisation (raster's ADR 0049). Their `artwork` archetype gains 1.2× from a
...
//! cargo run --release -p render-raster --example rect_and_residue_census -- <file.pdf>…
//! ```
//!
//! With no argument it walks the pdf.js corpus's first pages at the scale
//! `render-raster/tests/corpus.rs` renders them at, which is the population every other
...
fn main() {
    ...
    let mut raster = QuorraRasterizer::new_headless().expect("an adapter");
    ...
    for path in &files {
        ...
        if raster.rasterize(&list, target).is_err() {
        ...
        let (regions, tiles) = raster.last_clip_residue();
