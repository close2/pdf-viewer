//! Are the resource identifiers this backend hands raster the same from one render of
//! one display list to the next?
//!
//! raster keys every glyph-lane tile on the outline identifier the fill names, and its
...
//! the atlas twice over and repacks — for ever, at period two. The raster developers
...
//! ([`render_raster::FrameCost::uploads`] — cache misses plus transients), whether the
...
//! this program's: at 1 MiB the oscillation appears within two frames, at raster's default it
...
fn main() {
    ...
    // rendered alone takes many frames to fill it, which is why the raster developers reported
    // the oscillation at 1 MiB: a smaller atlas reaches the same steady state sooner, it does
    // not create it.
    let default_atlas = raster_gpu::Options::default().atlas_budget;
    ...
    let mut backend = QuorraRasterizer::with_options(&raster_gpu::Options {
        atlas_budget,
