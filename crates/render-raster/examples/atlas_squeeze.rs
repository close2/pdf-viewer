...
//! is raster's **atlas** — whose budget is its own `Options::atlas_budget`, separate from the
...
fn main() {
    ...
    let default = raster_gpu::Options::default();
    ...
    for (name, coverage) in [
        ...
        ("gpu lane", raster_gpu::Coverage::Gpu),
    ] {
    ...
    for budget in [
        ...
        let mut backend = match QuorraRasterizer::with_options(&raster_gpu::Options {
            atlas_budget: budget,
            coverage: raster_gpu::Coverage::Gpu,
            ..render_raster::options()
        }) {
            Ok(backend) => backend,
            Err(error) => {
