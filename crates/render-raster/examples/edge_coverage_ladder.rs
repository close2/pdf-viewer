...
fn main() {
    let mut raster = render_raster::QuorraRasterizer::new_headless().expect("an adapter");
    ...
    println!("  shape     cpu     raster          cpu     raster");
    for rung in 0_i16..=20 {
        ...
        let mut read = |list: &DisplayList, x: usize, y: usize| {
            ...
            let theirs = raster.rasterize(list, target).expect("drawn");
