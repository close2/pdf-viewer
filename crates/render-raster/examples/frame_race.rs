...
fn main() {
    ...
    for scale in [1.0_f32, 1.9008] {
        ...
        let mut raster = render_raster::QuorraRasterizer::new_headless().expect("adapter");
        let quorra_ms = fastest(&mut raster, &list, target);
        println!(
            "page 6 @{scale} ({}x{}): cpu {cpu_ms:.2} ms, vello {vello_ms:.2} ms, raster {quorra_ms:.2} ms",
