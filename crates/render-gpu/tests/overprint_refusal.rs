...
//!
//! `render-cpu` computes the mode (ADR 1157); Vello's `Mix` has only Table 135's sixteen, and the one value the clause's first bullet gives that is not the source
...
/// The GPU backend, or a panic naming why the whole suite should fail rather than skip.
fn gpu() -> GpuRasterizer {
    GpuRasterizer::new_headless().unwrap_or_else(|e| {
        panic!(
