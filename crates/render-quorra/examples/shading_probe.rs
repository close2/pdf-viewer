...
use pdf_render::{Rasterizer as _, ShadingKind, TargetSpec};
...
fn main() {
    ...
    let path = if path.exists() { path } else {
        // corpus location
        std::path::Path::new("/home/cl/projects/pdf-viewer/doc/pdf.js/test/pdfs/issue10572.pdf").into()
    };
    ...
    println!("target {}x{} transform {:?}", target.width, target.height, target.transform);
    ...
    for (i, command) in list.commands().iter().enumerate() {
        if let Some(bounds) = command.device_bounds(target.transform) {
            if bounds.min.x <= probe.0 && probe.0 <= bounds.max.x
                && bounds.min.y <= probe.1 && probe.1 <= bounds.max.y
            {
                match command {
                    pdf_render::Command::Fill { paint: pdf_render::Paint::Shading(shading), transform, .. } => {
                        println!("#{i}: Fill shading transform={:?} cmd-transform={transform:?}", shading.transform);
                        match shading.kind.as_ref() {
                            ShadingKind::Axial { start, end, ramp, extend } => println!(
                                "  Axial start=({},{}) end=({},{}) stops={} extend={extend:?}",
                                start.x, start.y, end.x, end.y, ramp.stops.len()
                            ),
                            ShadingKind::Radial { .. } => println!("  Radial"),
                            ShadingKind::Sampled { domain, width, height, .. } => println!(
                                "  Sampled domain={domain:?} grid={width}x{height}"
                            ),
                            ShadingKind::Mesh { triangles } => println!("  Mesh {} triangles", triangles.len()),
                            other => println!("  {other:?}"),
                        }
                    }
                    other => {
