...
fn a_conformance_answer_carries_its_own_extent() {
    ...
    // **There are four states, and this test asserted two until the others were built.** A
    ...
    // UCR and UCR2 functions", which no document can fail — or named as one a published
    // clarification places outside validation altogether. Folding any of them into the debt told
    // a reader that a validator owed work it does not owe, and made every coverage figure
    // understate itself: about twenty rows for the third state alone.
    assert_eq!(
        report.checked()
            + report.unchecked().count()
            + report.processor_obligations().count()
            + report.outside_validation().count(),
        report.judgements.len(),
        "every requirement is checked, owed, a processor's, or outside validation — and is \
         exactly one of them"
    );
    assert!(
