...
section_vfs() {
    ...
    # **`viewer-confined`, not `pdf-vfs`.** The nine-hundred-and-nineteenth session moved this
    # suite to the other confined program and `doc/verify.md` records the new invocation; this
    # line kept the old one, which names a test target that no longer exists — so the section
    # failed for a reason that had nothing to do with what it measures. Found by the merge that
    # brought both rounds together (session 922).
    run "a document of each awkward class, from every corpus on the disk, through the worker" \
        '^view-awkward:|^vfs-awkward:' \
