...
# gone since ADR 0707, which made it true the other way; the trap is not, and it is why the strip
# stays.)
#
...
Query:Collection|not a debt, since ADR 0707|"[i]f this dictionary is present in a PDF document, the interactive PDF processor shall present the document as a portable collection" (§12.3.5) — a shall addressed to a viewer, and the sharpest clause this reading ever named. All three windows present one now: viewer_host::panel::collection_rows is the mapping and both native hosts ask this question before Query::Attachments, because a collection is those same files arranged. A window that names neither would be a window with no files panel at all.
READING
}
